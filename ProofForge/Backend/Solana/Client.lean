...
def renderWithIdl (idl : String) : String :=
  String.intercalate "\n" [
    ...
    "export type InstructionArgs = Record<string, bigint | number | boolean | Uint8Array | number[]>;",
    ...
    "    } else if (param.encoding === \"raw-bytes\") {",
    "      const bytes = value instanceof Uint8Array ? value : Uint8Array.from(value as number[]);",
    "      if (bytes.length !== param.byteSize) {",
    "        throw new Error(`ProofForge raw-bytes param ${param.name}: expected ${param.byteSize} bytes, got ${bytes.length}`);",
    "      }",
    "      data.set(bytes, param.offset);",
    "    } else {",
    ...
    "}",
    "",
    "function hasBudgetValue(value: bigint | number | null | undefined): value is bigint | number {",
    "  return value !== null && value !== undefined;",
    "}",
    "",
    "export function computeBudgetInstructions(name: InstructionName) {",
    "  const instruction = instructionByName(name);",
    "  const instructions: TransactionInstruction[] = [];",
    "  for (const budget of instruction.computeBudget ?? []) {",
    "    if (hasBudgetValue(budget.unitLimit)) {",
    "      instructions.push(ComputeBudgetProgram.setComputeUnitLimit({ units: Number(budget.unitLimit) }));",
    "    }",
    "    if (hasBudgetValue(budget.unitPriceMicroLamports)) {",
    "      instructions.push(ComputeBudgetProgram.setComputeUnitPrice({ microLamports: budget.unitPriceMicroLamports }));",
    "    }",
    "  }",
    "  return instructions;",
    "}",
    ...
    "}",
    "",
    "export function createTransactionInstructions(",
    "  programId: PublicKey,",
    "  name: InstructionName,",
    "  accounts: AccountPubkeys,",
    "  args: InstructionArgs = {},",
    ") {",
    "  return [",
    "    ...computeBudgetInstructions(name),",
    "    createInstruction(programId, name, accounts, args),",
    "  ];",
    "}",
    ""
  ]
