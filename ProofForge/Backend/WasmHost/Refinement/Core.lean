...
def emitWatExportsOk (obligation : TraceObligation) : Bool :=
  match ProofForge.Backend.WasmHost.EmitWat.renderModule obligation.module with
...
def ArtifactSurfaceObligation.hostImportSignaturesOk
  ...
  match ProofForge.Backend.WasmHost.EmitWat.lowerModule obligation.module with
  | .ok wasm =>
