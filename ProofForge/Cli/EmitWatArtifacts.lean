...
def writeEmitWatDeployManifest
  ...
  let manifest := jsonObject #[
    ...
    -- N1.7: offline vs broadcast honesty (aligned with EVM deployment.broadcast).
    -- `validation.deployManifest=passed` only means this JSON was written, not that
    -- a NEAR account was created or a transaction was broadcast.
    ("deployment", jsonObject #[
      ...
      ("nearSandbox",
        if targetId == ProofForge.Target.wasmNear.id then
          jsonString "not-generated"
        else
          "null"),
      ("note", jsonString (
        if targetId == ProofForge.Target.wasmStellarSoroban.id then
          "EmitWat Soroban Spike output is locally executable through runtime/offline-host; Stellar account deployment / contract-spec publish / broadcast is not generated by this manifest."
        else if targetId == ProofForge.Target.wasmCosmWasm.id then
          "EmitWat CosmWasm output is locally executable through runtime/offline-host; chain upload, instantiate, and broadcast artifacts are not generated by this manifest. validation.deployManifest=passed only records that this file was emitted."
        else
          "EmitWat target-first output is locally executable through runtime/offline-host; NEAR account deployment, network broadcast, and sandbox deploy artifacts are not generated by this manifest. validation.deployManifest=passed only records that this file was emitted."))
    ])
  ]
...
def writeEmitWatArtifactMetadata
  ...
  let (contractSpecOutput, nearClientOutput, unifiedClientOutput) ←
    if opts.fromNewSurface then
      writeWasmHostContractSidecars targetId outputDir spec
    else if targetId == ProofForge.Target.wasmStellarSoroban.id then
      pure (outputDir / s!"{spec.name}.contract-spec.json",
        outputDir / ProofForge.Contract.Client.sorobanWrapperPath,
