...
namespace Tests.Quint.ArrayReplay
...
def main : IO UInt32 := do
  let qntPath := "build/quint/ArrayReplay.qnt"
  let itfPath := "build/quint/ArrayReplay.itf.json"
  ...
  | .error err =>
      ...
      return 1
  | .ok trace =>
      match Replay.replayTrace ProofForge.IR.Examples.ArrayProbe.emitWatStorageModule trace with
      | .error err =>
...
end Tests.Quint.ArrayReplay

def main : IO UInt32 := Tests.Quint.ArrayReplay.main