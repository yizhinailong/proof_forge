...
namespace Tests.Quint.MapHashPathAssignReplay
...
def main : IO UInt32 := do
  let qntPath := "build/quint/MapHashPathAssignReplay.qnt"
  let itfPath := "build/quint/MapHashPathAssignReplay.itf.json"
  ...
  | .error err =>
      ...
      return 1
  | .ok trace =>
      match Replay.replayTrace ProofForge.IR.Examples.MapProbe.emitQuintHashPathAssignModule trace with
      | .error err =>
...
end Tests.Quint.MapHashPathAssignReplay

def main : IO UInt32 := Tests.Quint.MapHashPathAssignReplay.main