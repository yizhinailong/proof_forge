...
namespace Tests.Quint.MapNestedDynamicPathReplay
...
def main : IO UInt32 := do
  let qntPath := "build/quint/MapNestedDynamicPathReplay.qnt"
  let itfPath := "build/quint/MapNestedDynamicPathReplay.itf.json"
  ...
  | .error err =>
      ...
      return 1
  | .ok trace =>
      match Replay.replayTrace ProofForge.IR.Examples.MapProbe.emitQuintNestedDynamicPathModule trace with
      | .error err =>
...
end Tests.Quint.MapNestedDynamicPathReplay

def main : IO UInt32 := Tests.Quint.MapNestedDynamicPathReplay.main