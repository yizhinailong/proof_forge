...
namespace Tests.Quint.MapPathAssignReplay
...
def main : IO UInt32 := do
  let qntPath := "build/quint/MapPathAssignReplay.qnt"
  let itfPath := "build/quint/MapPathAssignReplay.itf.json"
  ...
  | .error err =>
      ...
      return 1
  | .ok trace =>
      match Replay.replayTrace ProofForge.IR.Examples.MapProbe.emitQuintPathAssignModule trace with
      | .error err =>
...
end Tests.Quint.MapPathAssignReplay

def main : IO UInt32 := Tests.Quint.MapPathAssignReplay.main