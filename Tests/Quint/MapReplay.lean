...
namespace Tests.Quint.MapReplay
...
def main : IO UInt32 := do
  let qntPath := "build/quint/MapReplay.qnt"
  let itfPath := "build/quint/MapReplay.itf.json"
  ...
  | .error err =>
      ...
      return 1
  | .ok trace =>
      match Replay.replayTrace ProofForge.IR.Examples.MapProbe.emitQuintStorageModule trace with
      | .error err =>
...
end Tests.Quint.MapReplay

def main : IO UInt32 := Tests.Quint.MapReplay.main