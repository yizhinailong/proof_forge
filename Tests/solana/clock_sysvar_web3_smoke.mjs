...
async function pollTransaction(connection, signature) {
  ...
  throw new Error(`transaction not available for ${signature}`);
}
...
async function main() {
  ...
  const tx = await pollTransaction(connection, signature);
  ...
  const recordedSlot = readU64LE(account.data);
  if (recordedSlot === 0n) {
    throw new Error("recorded Clock.slot was zero");
  }

  const transactionSlot = BigInt(tx.slot);
  const delta = recordedSlot > transactionSlot
    ? recordedSlot - transactionSlot
    : transactionSlot - recordedSlot;
  if (delta > 2n) {
    throw new Error(`Clock.slot ${recordedSlot} is too far from transaction slot ${transactionSlot}`);
  }
  ...
  console.log(JSON.stringify({
    ...
    signature,
    recordedSlot: recordedSlot.toString(),
    transactionSlot: transactionSlot.toString(),
    slotDelta: delta.toString(),
  }));
}
