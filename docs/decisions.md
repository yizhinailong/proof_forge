...
| D-027 | 2026-07-01 | Adopt **`EmitWat`** (LCNF → WAT text → `wat2wasm`) as the canonical Wasm-family backend; demote the Rust `near-sdk-rs` sourcegen to a **frozen v0 stopgap** | The Rust sourcegen route is architecturally inconsistent with the EVM backend (which lowers Lean's own LCNF via `EmitYul`) and couples to `near-sdk` macro conventions (revealed by the E0119 Borsh / missing-`&self` cargo-check failures). `EmitZig` (the prior canonical plan) is blocked on porting the full Lean runtime to Wasm. `EmitWat` mirrors `EmitYul` — LCNF lowering that intercepts `lean_<chain>_*` externs and lowers them to host imports, reusing the `EmitYul` object model (boxed scalars, bump allocator, refcount elision, no GC) which is safe because Wasm contract calls are atomic with fresh linear memory per invocation. The shared layer (lowering, object model, WAT scaffolding, `wat2wasm`) is written once for the whole family; only the per-chain host module + ABI serialization differ. Key spike risk: NEAR argument (de)serialization (JSON/Borsh), which `EmitYul` does not face for EVM calldata |
...
| Wasm host | `wasm-near`, `wasm-cosmwasm`, `wasm-stellar-soroban` (candidate, docs only), `wasm-icp-canister` (candidate, docs only) | Lean → LCNF → **`EmitWat`** (WAT) → `wat2wasm` + per-chain host bridge; Rust/CDK sourcegen used only as a frozen v0 stopgap ([D-027](targets/wasm-family.md)) |
