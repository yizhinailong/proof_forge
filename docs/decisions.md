...
| Wasm host | `wasm-near`, `wasm-cosmwasm`, `wasm-stellar-soroban` (candidate, docs only), `wasm-icp-canister` (candidate, docs only) | Portable IR → **`EmitWat`** (Wasm AST → WAT) → `wat2wasm` + per-chain host imports; Rust/CDK sourcegen used only as a frozen v0 stopgap ([D-027](targets/wasm-family.md)) |
