...
| D-050 | 2026-07-09 | **IR portability remediation (slice 1):** (1) add `StorageOwner` (`contract` \| `resource` \| `object`) orthogonal to `StateKind` shape, with `storage.resource` / `storage.object` capabilities; (2) rename EVM-named IR fields to neutral names (`paramAbiWords`, `proxyPattern?`) with compatibility aliases; (3) ship `ProofForge.IR.Portability` classifier that tags portable-core vs family-shared vs target-family-only constructors (EVM create/create2/STATICCALL/DELEGATECALL/fallback/receive, NEAR Promise ops, Move owners); (4) EVM rejects non-contract owners; Move Aptos/Sui accept explicit resource/object owners while still mapping portable `contract` scalar Counter MVP. Portable Counter/ValueVault keep `owner := .contract`. Follow-ups: migrate NEAR Promise constructors out of `Expr` into host-extension metadata (D-027), expand Move beyond Counter-shaped entrypoint-name templates, strip remaining EVM-shaped context fields from portable default. | IR was EVM-evolved (`paramEvmAbiWords`, `evmProxyPattern?`, missing object/resource ownership) while docs already described account_owned/object kinds. D-027 requires ≥2 families before portable constructors; this decision makes the inventory machine-checkable and unblocks honest Move ownership without breaking three-chain portable fixtures. Gate: `just ir-portability-smoke`. |
