...
| D-039 | 2026-07-03 | Plan the CLI product surface as `proof-forge build|emit|check --target <id> [--fixture <id>]` before testkit M4 binds to legacy flags | RFC 0009 defines the target-first surface, fixture registry, and legacy-flag alias/deprecation plan. Implementation is staged after the RFC is reviewed; no code changes until Workstream 29 M1 |
