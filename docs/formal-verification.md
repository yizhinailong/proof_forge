...
| `evm` (IR → Yul plan) | Counter and ValueVault obligations exist for IR trace + selector-dispatched Yul surface + executable Yul-subset trace (`calldataload`, `calldatasize`, `sstore`, `sload`, scalar arithmetic, `number`, `keccak256`, `log0`-`log4`, `mstore`, `return`); next, extend beyond scalar storage into assertions, maps, arrays, structs, and aggregate return/state shapes | Medium — the focused Yul-subset interpreter is in Lean; expanding coverage keeps `solc` out of the trusted path but not out of the build |
...
4. FV-4 EVM Yul-subset trace obligations for Counter and ValueVault: done for
   selector-dispatched scalar-storage paths; next expand the interpreter and
   obligations toward assertions, maps, arrays, structs, and aggregate
   return/state shapes.
