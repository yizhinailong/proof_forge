...
| Solana sBPF ↔ solanalib bridge (Scheme 1 + 2A/B + host) | `ProofForge/Backend/Solana/BpfEncode.lean`, `LabeledSbpf.lean` (default), `SolanaRefinement/*` (opt-in), `docs/solana-sbpf-solanalib-bridge.md`, `docs/portable-ir-semantics-anchor.md` | **Always from Portable IR.** Encode/labeled/lift/verify; Counter core-tail + full EmitSBPF host; IR↔full-host pointwise paired simulation; **ValueVault default scenario on full host** (multi-field state projection + PF differential). Universal all-input host refinement still open. Gates: `just solana-bpf-encode-smoke`, `just solana-solanalib-adapter` |
