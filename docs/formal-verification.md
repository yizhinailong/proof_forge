...
| **C-proof — Refinement theorems** | Universally-quantified Lean theorems relating IR semantics to target semantics for all inputs in a declared supported fragment | Today: structural FV-1 routing theorems are universal; `runTraceListGen_sound` is universal over arbitrary machine states and is smoke-instantiated for IR plus the EVM/Yul, Solana sBPF, and Wasm/NEAR target runners; `traceSimulation_lift` is the shared induction lemma that lifts per-entrypoint IR/target simulation into whole-trace observable equality; the Counter proof-fragment path instantiates it against a tiny `counter-model` target semantics, gated by `TargetSemantics.supportedFragment`; the real Solana/Wasm target runners now have Counter paired-step soundness theorems (`counter_sbpf_trace_simulation_sound_checked`, `counter_wasm_trace_simulation_sound_checked`) proving matching observable arrays and final storage relation for the fixed Counter trace; the EVM bytecode seam now targets `powdr-labs/evm-semantics` but keeps the default build mathlib-free | It does **not** prove EVM/Solana/Wasm bytecode semantics yet. The Counter target model is a proof skeleton, not a chain VM. The Solana/Wasm paired-step soundness is still fixed-trace, not all-input/full-fragment refinement. Full IR↔EVM bytecode refinement still requires the opt-in powdr/mathlib target and the real simulation relation; full Solana/NEAR refinement still needs real per-entrypoint simulation lemmas over their target interpreters plus the supported-fragment boundary | `ProofForge/IR/StepSemantics.lean`, `ProofForge/IR/CounterSemantics.lean`, `ProofForge/Backend/Refinement/Core.lean`, `ProofForge/Backend/Refinement/CounterUniversal.lean`, `ProofForge/Backend/Evm/EvmBytecodeSemantics.lean`, `ProofForge/Target/Formal.lean` |
...
checks against the IR reference (Tier C-diff), and has started the C-proof
track with a Counter-only supported fragment against a tiny proof target model.
Full chain-native IR↔target refinement remains in progress." Avoid
