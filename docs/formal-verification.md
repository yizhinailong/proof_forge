...
| **C-diff — Differential testing** | IR reference trace replayed against the EVM Yul-subset interpreter (including mathlib-free IR↔Yul host paired simulation), external `solc --strict-assembly` verification of Counter Yul→bytecode against the powdr witness, the Counter + ValueVault scalar/event slices of the in-Lean Wasm/NEAR and Solana sBPF interpreters, the focused Wasm and Solana fixed-array/u64-map storage slices, the NEAR offline host, and capability routing soundness at the `resolveSpec` boundary | For the **fixed Counter/ValueVault/probe scenarios**, the observable return values and event logs produced by the target artifact match those produced by the IR reference semantics; FV-1 confirms that any spec that resolves to `.ok plan` on a profile yields a plan whose capabilities are all supported | It does **not** prove anything for inputs outside the fixed scenarios (`native_decide` is pointwise, not universal); EVM coverage stops at the Yul subset (solc is not in the trusted path but is externally checked for Counter); the Wasm and Solana executable traces cover Counter + ValueVault scalar/event plus focused fixed-array/u64-map storage probes, while CPI/PDA, broad host/syscall semantics, hash maps, nested paths, dynamic arrays, and aggregate array elements stay outside these in-Lean target interpreters | `ProofForge/Backend/Evm/Refinement.lean`, `ProofForge/Backend/Evm/YulHostRefinement.lean`, `scripts/evm/yul-compiler-counter-smoke.sh`, `ProofForge/Backend/WasmHost/Refinement.lean`, `ProofForge/Backend/Solana/Refinement.lean`, `ProofForge/Target/Formal.lean` |
...
1. **EVM bytecode lane** trusts **powdr `stepF`** + `native_decide` (the pinned
   ...
   Yul→bytecode `solc` hop is **not** proven, but it is now externally checked
   for Counter: `just evm-yul-compiler-counter-smoke` compiles the emitted Yul
   with `solc --strict-assembly` and verifies the runtime code matches the
   powdr witness embedded in `EvmRefinement`. The Counter IR↔powdr delivery
   boundary is pinned by `counterCompiledPowdr_ir_and_target_trace_match`
   (same `TraceObligation` passes IR reference trace and powdr bytecode trace)
   plus the CLI runtime-bytecode witness check. Gates:
   `just evm-powdr-counter-refinement-smoke`, `just evm-powdr-counter-runtime`,
   `just evm-yul-compiler-counter-smoke`.
