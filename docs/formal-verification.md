...
| **C-diff — Differential testing** | IR reference trace replayed against the EVM Yul-subset interpreter, the Counter + ValueVault scalar/event slice of the in-Lean Wasm/NEAR interpreter, the NEAR offline host, and the Counter slice of the in-Lean Solana sBPF interpreter; capability routing soundness at the `resolveSpec` boundary | For the **fixed Counter/ValueVault/probe scenarios**, the observable return values and event logs produced by the target artifact match those produced by the IR reference semantics; FV-1 confirms that any spec that resolves to `.ok plan` on a profile yields a plan whose capabilities are all supported | It does **not** prove anything for inputs outside the fixed scenarios (`native_decide` is pointwise, not universal); EVM coverage stops at the Yul subset (solc is not in the trusted path but not proven); the Wasm executable trace covers the Counter + ValueVault scalar/event subset, while Solana executable trace still covers only the Counter scalar-storage subset | `ProofForge/Backend/Evm/Refinement.lean`, `ProofForge/Backend/WasmNear/Refinement.lean`, `ProofForge/Backend/Solana/Refinement.lean`, `ProofForge/Target/Formal.lean` |
...
| `solana-sbpf-asm` | **Executable-trace Counter subset exists**: the Counter IR observable trace, rendered assembly entrypoint labels, lowered structured `AstNode` interpreter trace, scalar account-data relation `R`, and revert-rollback invariant are all `native_decide`-checked. The in-Lean sBPF interpreter covers the Counter scalar-storage slice (dispatch, account-validation path, direct account-data loads/stores, ALU/load/store/jump/exit, and `sol_set_return_data`). Full sBPF semantics (CPI/syscalls/account model) stays in the external differential gate (Mollusk/Surfpool) | Medium for the interpreter slice (reuses existing `Asm.lean` structured AST); research track for fuller account/syscall coverage |
...
4. FV-4 backend trace obligations: scalar EVM IR traces are done for
   ...
   Counter + ValueVault scalar/event subset has an in-Lean executable Wasm
   trace; next, deepen that boundary toward maps, arrays, and richer
   Wasm/offline-host semantics.
