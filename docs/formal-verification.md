...
| Solana sBPF ↔ solanalib bridge (Scheme 1 + 2A/B + host) | `ProofForge/Backend/Solana/BpfEncode.lean`, `LabeledSbpf.lean` (default), `SolanaRefinement/*` (opt-in), `docs/solana-sbpf-solanalib-bridge.md`, `docs/portable-ir-semantics-anchor.md` | **Always from Portable IR.** EmitSBPF `AstNode` → labeled register asm → `BpfBinBytes`; opt-in direct lift + `verifyInstr` + `step_ne_err`; **Counter core-tail host bridge** runs `initialize`/`increment` tails on a solanalib `step` driver with word↔byte memory, matching ProofForge finals on `r0` and `countOff`. Full EmitSBPF program + syscall host still open. Gates: `just solana-bpf-encode-smoke`, `just solana-solanalib-adapter` |
