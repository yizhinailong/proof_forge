...
| **C-proof — Refinement theorems** | Universally-quantified Lean theorems relating IR semantics to target semantics for all inputs in a declared supported fragment | Today: structural FV-1 routing theorems are universal; `runTraceListGen_sound` is universal over arbitrary machine states and is smoke-instantiated for IR plus the EVM/Yul, Solana sBPF, and Wasm/NEAR target runners; the Counter proof-fragment path proves all Counter call lists against a tiny `counter-model` target semantics, gated by `TargetSemantics.supportedFragment`; the EVM bytecode seam now targets `powdr-labs/evm-semantics` but keeps the default build mathlib-free | It does **not** prove EVM/Solana/Wasm bytecode semantics yet. The Counter target model is a proof skeleton, not a chain VM. Full IR↔EVM bytecode refinement still requires the opt-in powdr/mathlib target and the real simulation relation; full Solana/NEAR refinement needs research-grade formal sBPF/host semantics | `ProofForge/IR/StepSemantics.lean`, `ProofForge/IR/CounterSemantics.lean`, `ProofForge/Backend/Refinement/Core.lean`, `ProofForge/Backend/Refinement/CounterUniversal.lean`, `ProofForge/Backend/Evm/EvmBytecodeSemantics.lean`, `ProofForge/Target/Formal.lean` |
