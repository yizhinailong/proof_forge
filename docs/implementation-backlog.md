...
- Done for EVM: add an Anvil deploy smoke that sends generated Counter
  `.init.bin` with `cast send --create`, records constructor args plus a
  `proof-forge-deploy-run.json` artifact, validates the receipt/deployed
  address/runtime-code match, and exercises the Counter lifecycle over
  JSON-RPC.
