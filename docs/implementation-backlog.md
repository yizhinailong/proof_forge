...
- Done for EVM: add an Anvil deploy smoke that sends generated Counter
  ...
  constructor args plus a `proof-forge-deploy-run.json` artifact, validates the
  receipt/deployed address/runtime-code match, and exercises the Counter
  lifecycle over JSON-RPC.
...
- Local Anvil deployment can consume the generated deploy manifest and initcode,
  ...
  matches the generated bytecode even when the initcode includes an ABI-encoded
  constructor-argument tail with a recorded static constructor ABI schema.
