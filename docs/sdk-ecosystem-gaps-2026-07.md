...
| ERC-4626 (vault standard) | Partial | **Call** peer: `IERC4626` / `external_vault`. **Deploy body:** `Stdlib.ERC4626` 1:1 with IERC20 **transferFrom pull / transfer push** (`just product-erc4626-vault`). Non-1:1 rates / fees still open | P1 |
| ERC-2612 (permit) | Partial | **Call** peer + **deploy body** (`ERC20Permit`). TokenSpec EVM plans `permit` with `crypto.ecrecover` + stdlib note; legacy Token/Evm.lean hand Yul still without permit entry | P1 |
