...
| ERC-4626 (vault standard) | Partial | **Call** peer: `IERC4626` / `external_vault`. **Deploy body:** `Stdlib.ERC4626` **pro-rata** (`shares = assets * supply / totalAssets`, empty 1:1) + IERC20 **transferFrom pull / transfer push** (`just product-erc4626-vault`). Fees / fee-on-transfer / preview rounding still open | P1 |
