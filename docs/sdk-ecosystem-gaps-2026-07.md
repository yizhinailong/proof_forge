...
| ERC-4626 (vault standard) | Partial | **Call** deployed vault: `Protocols.Evm.IERC4626` + Product `external_vault` (`just product-protocol-vault`). Layer C *body* / TokenSpec vault still missing | P1 |
| ERC-2612 (permit) | Partial | **Call** external `permit`: `Protocols.Evm.IERC20Permit`. TokenSpec `permit` still **rejects** on EVM for *being* the token (nonces/DOMAIN_SEPARATOR body deferred) | P1 |
