...
| ERC-4626 (vault standard) | Partial | **Call** peer: `IERC4626` / `external_vault`. **Deploy body:** `Stdlib.ERC4626` **pro-rata** + **entry/exit feeBps** + **FOT balance-delta** deposit/mint + net `mint`/`preview*` + IERC20 pull/push (`just product-erc4626-vault`). Exit push still assumes full transfer amount | P1 |
