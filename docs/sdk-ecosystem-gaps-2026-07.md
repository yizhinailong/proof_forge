...
| Storage packing | Deferred (D-051) | EVM keeps one 32-byte slot per `StateDecl` field; NEAR/WasmHost already packs multiple scalars into one storage key (`__pf_pack_*`). EVM consecutive-word packing not justified for current product surface (Counter/ValueVault/stdlib). Future lint/diagnostic may warn about gas waste. Revisit when a product example hits SSTORE gas limits. | P2 |
