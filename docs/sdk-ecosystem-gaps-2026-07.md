...
| ERC-4626 (vault standard) | Covered (honest subset) | **Call** peer: `IERC4626` / `external_vault`. **Deploy body:** `Stdlib.ERC4626` pro-rata + entry/exit feeBps + **FOT balance-delta on pull and push** + net mint/preview (`just product-erc4626-vault`). Recipient-side FOT (user receives less) not re-measured | P1 |
