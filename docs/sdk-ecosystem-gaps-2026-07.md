...
| ERC-4626 (vault standard) | Partial | **Call** peer: `IERC4626` / `external_vault` (+ preview selectors). **Deploy body:** `Stdlib.ERC4626` **pro-rata** + `preview*` (= convert floor) + IERC20 pull/push (`just product-erc4626-vault`). Fees / fee-on-transfer / asymmetric preview rounding still open | P1 |
