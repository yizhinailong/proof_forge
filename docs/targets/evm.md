...
|---|---|
| `storage.scalar` | `contract_source state`; portable IR `Bool`/`U8`/`U32`/`U64`/`U128`/`Address`/`Hash` scalar storage read/write with Solidity-style storage packing (small scalars share 32-byte slots via bit-shift masking), scalar storage compound assignment for numeric words, flat scalar storage struct field read/write, and whole flat scalar storage struct read/write; artifact metadata includes `storageLayout` JSON with per-state slot/byteOffset/byteWidth |
...
- `deployment.broadcast: not-generated`, because transaction signing,
  ...
  generated by `proof-forge build`. Use `proof-forge deploy --target evm` to
  broadcast on `anvil-local` or emit a documented testnet deploy plan.
...
`scripts/evm/deploy-plan-smoke.sh` records
`build/evm-deploy-plan-smoke/Counter.proof-forge-deploy-plan.json` for the
documented `robinhood-chain-testnet` profile without live broadcast.
