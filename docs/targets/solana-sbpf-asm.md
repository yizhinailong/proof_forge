...
| CPI (`sol_invoke_signed_c`, `sol_invoke_signed_rust`) | SDK metadata, entry actions, and helper emission exist; System Program transfer/create-account, Memo Program one-word raw payloads, Associated Token `create_idempotent`, SPL Token helpers, and Token-2022 helpers pack C `SolInstruction`, standard instruction data bytes, `SolAccountMeta[]`, bound `SolAccountInfo[]`, signer seed tables, and decoded scalar entrypoint parameters; System transfer/create-account, Memo, Associated Token `create_idempotent`, SPL Token `transfer_checked`, SPL Token `mint_to`/`burn`/`approve`/`revoke`, SPL Token `set_authority`, SPL Token `close_account`, the Token-2022 transfer-fee and non-transferable plans, generated Token-2022 direct CPI extension fixture, Token-2022 Pausable direct CPI, and the generated Token-2022 transfer-hook execute path have Surfpool/Rust live behavior gates; System transfer, System `create_account`, SPL Token `transfer_checked`, SPL Token `mint_to`/`burn`/`approve`/`revoke`, and SPL Token `set_authority` now have checked-in Pinocchio reference contract/manifest gates included in `just solana-light`, plus dual-deploy live-equivalence harnesses gated on Solana rustc availability | Make the Pinocchio live gates pass in CI/local toolchains, add Token-2022 reference coverage, add arbitrary-length Memo payloads, and extend remaining SPL/Associated Token helper live-equivalence |
