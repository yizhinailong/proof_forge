...
| CPI (`sol_invoke_signed_c`, `sol_invoke_signed_rust`) | SDK metadata + extension helpers pack C `SolInstruction` + metas/infos; **portable** `crosscall.invoke` uses a **dedicated CPI stack frame** (`PortableCrosscall`) that forwards the full instruction account list, capped at `MAX_PORTABLE_CPI_ACCOUNTS` (= `min(64, 40)` — full 64 infos + 64 ptr table exceed the 4 KiB sBPF stack). Account pointer table holds 64 slots. Schema sizes above the cap are rejected at lower time. Source.Solana hand-tuned CPI keeps the legacy Extension offsets. | Heap-backed infos for full 64; Pinocchio live CI; remaining SPL live-equivalence |
