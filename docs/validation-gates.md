...
| CI baseline | `.github/workflows/ci.yml` `build-test` job | GitHub Actions Ubuntu, elan, Foundry stable, `solc` 0.8.30 | Clean-environment `lake build`, Psy golden source snapshots for Counter/ContextProbe/HashProbe/MapProbe/AssertProbe/LoopProbe/ArrayProbe/StructProbe/StructArrayProbe, EVM compile, and Foundry smoke | Optional Dargo target smokes, metadata validation, non-Ubuntu behavior |
