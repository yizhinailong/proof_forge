...
| Psy HashProbe IR smoke | `scripts/psy/hash-smoke.sh` | `dargo` on `PATH`; `python3`; `psyup install 0.1.0` is known-good on macOS arm64 | Psy IR lowers `Hash`, typed hash let-bindings, `hash`, and `hash_two_to_one` to `.psy`, matches the golden fixture, passes `dargo test --file`, produces non-empty DPN JSON and ABI JSON, returns the expected four-Felt hash outputs through `dargo execute`, and writes `proof-forge-artifact.json` | Maps, storage maps, deploy JSON, live Psy node/prover behavior |
| CI baseline | `.github/workflows/ci.yml` `build-test` job | GitHub Actions Ubuntu, elan, Foundry stable, `solc` 0.8.30 | Clean-environment `lake build`, Psy golden source snapshots for Counter/ContextProbe/HashProbe, EVM compile, and Foundry smoke | Optional Dargo target smokes, metadata validation, non-Ubuntu behavior |
