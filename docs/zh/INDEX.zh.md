...
ProofForge 是一个 Lean 优先的多链智能合约平台。在 2026-07 分支合并后，主干包含 EVM 基线，以及位于统一的可移植 IR 和能力注册表之下的 Solana (sBPF 汇编)、NEAR (EmitWat)、Sui (Counter MVP)、CosmWasm 和 Aptos (Counter spike)、Psy/DPN、Aleo Leo 和 Cloudflare Workers (TypeScript spike) 后端。

**当前阶段：** 三个主要产品链 `solana-sbpf-asm`、`evm` 和 `wasm-near` 的 Gate P0 已关闭。下一个加固路径是从旧版标志到 `proof-forge build|emit|check --target ...` 的 CLI M3/M4 迁移；Tier-1 M3/M4 工作在该清理工作完成后进行。

## 文档地图

| 如果你是…… | 从这里开始 | 然后阅读 |
|---|---|---|
| 新贡献者 | 本页面 + [README](../README.md) + [入职指南](onboarding.md) | [可移植三目标教程](tutorials/portable-contract-three-targets.md)，[验证门控](validation-gates.md)，[待办事项](implementation-backlog.md) |
| 正在实现后端 | [RFC 0002](rfcs/0002-target-implementation-design.md) | [决策](decisions.md)，[可移植 IR](portable-ir.md)，目标说明 |
| 正在审查设计 | [审查清单](review-checklist.md) | RFC，[能力注册表](capability-registry.md)，[共享场景](shared-scenario.md) |
...
- [教程：从零走 Product 路径](tutorials/portable-shared-path.zh.md)：Counter → Ownable → Token → Remote（`just product`，T4.2）。
- [示例与测试分类](examples-and-tests-taxonomy.zh.md)：Product vs Backend vs IR。
- [产品北极星（中文）](product-north-star.md)：只写业务 + `--target` 物化。
...
- [RFC 0005: Solana sBPF 汇编后端](rfcs/0005-solana-sbpf-assembly-backend.md) (已接受 —— 规范的 Solana 路线，D-026)
- [RFC 0006: 多链 Token SDK](rfcs/0006-multichain-token-sdk.md) (草案)
- [RFC 0007: 统一的 Rust 测试框架](rfcs/0007-unified-rust-test-framework.md) (草案 —— 基于 revm/Mollusk/wasmtime 的测试套件场景)
- [RFC 0008: 链解耦的分配器抽象](rfcs/0008-allocator-abstraction.md) (草案 —— 每个目标绑定一个分配器模型)

## 工程

_- [开发标准](development-standards.md)：贡献者规则与单一事实来源地图。
- [入职指南](onboarding.md)：本地设置路径、编辑器说明以及新贡献者的最小验证循环。
- [Quint 模型生成](quint.md)：从可移植 IR 发射可执行状态机模型，进行模拟、模型检查并重放 MBT 追踪。
- [开发日志](development-log.md)：带有验证说明和后续步骤的里程碑日志。
- [编写模型](authoring-model.md)：学习源、`contract_source` 以及内部 `ContractSpec` 边界。
- [验证门](validation-gates.md)：可运行的门和工具先决条件。
- [形式化验证路线图](formal-verification.md)：现有的形式化锚点和阶段性证明目标。
- [Solana sBPF 可执行追踪](solana-sbpf-executable-trace.md)：Lean 内的 Counter + ValueVault 标量/事件，以及用于直接 sBPF 汇编的固定数组/u64-map 存储目标语义。
- [WASM 可执行追踪](wasm-executable-trace.md)：Lean 内的 Counter + ValueVault 标量/事件，以及用于 EmitWat/NEAR 的固定数组/u64-map 存储目标语义。
...
- [产品编写架构](product-authoring-architecture.md)：业务意图 vs 链具象化；Phase A–C 状态。
- [可移植 SDK 统一计划 (2026-07-09)](superpowers/plans/2026-07-09-portable-sdk-unification.md)：仅针对 EVM · Solana · NEAR · Soroban 的下一波多阶段任务（策略 · Token · remote）。
...
- [目标说明](targets/README.md)：每个家族的 research 和 spike 计划。
  ...
  - [Solana sBPF](targets/solana-sbf.md) (已废弃的 Zig/sbpf-linker 路线)
...
- NEAR：`proof-forge emit|build --target wasm-near --format wat` 通过 Wasm AST 将可移植 IR 降级为 WAT，具有形式化追踪义务 (`Tests/NearWasmFormal.lean`)、目标优先元数据和离线宿主冒烟测试。
- Psy/DPN、Aleo Leo 和 Cloudflare Workers 从可移植 IR 固定装置发射目标源代码；各门控的工具先决条件请参阅 [validation-gates.md](validation-gates.md)。
