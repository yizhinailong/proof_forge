...
| 如果你是… | 从这里开始 | 然后阅读 |
|---|---|---|
| 新贡献者 | 本页面 + [README](../README.md) + [入职指南](onboarding.md) | [可移植三目标教程](tutorials/portable-contract-three-targets.md)，[验证门](validation-gates.md)，[待办事项](implementation-backlog.md) |
| 实现后端 | [RFC 0002](rfcs/0002-target-implementation-design.md) | [决策](decisions.md)，[可移植 IR](portable-ir.md)，目标笔记 |
| 审查设计 | [审查清单](review-checklist.md) | RFC，[能力注册表](capability-registry.md)，[共享场景](shared-scenario.md) |
...
适用于演示和入职培训的可编辑手绘风格图表 —— 可在 [excalidraw.com](https://excalidraw.com) 或通过编辑器内的 Excalidraw 插件打开：

- [图表目录](diagrams/README.md) —— 七个 `.excalidraw` 文件，涵盖平台概览、编译流水线、多目标 Counter、能力路由、开发者工作流、代码库布局以及目标全景。

## 规范与决策

- [设计决策](decisions.md)：已确定的架构选择和路线图摘要。
- [可移植合约 IR](portable-ir.md)：IR 草图和第一阶段验收标准。
- [RFC 0003: 可移植 IR 与运行时](rfcs/0003-portable-ir-and-runtime.md)：详细的 IR/能力/运行时草案。
- [RFC 0004: EVM 语义计划与 Yul AST 边界](rfcs/0004-evm-semantic-plan.md)：位于可移植 IR 与底层 Yul 语法之间的目标语义 EVM 计划层。
...
- [编写模型](authoring-model.md)：学习源代码、`contract_source` 以及内部 `ContractSpec` 边界。
- [验证门禁](validation-gates.md)：可运行的门禁与工具先决条件。
- [形式化验证路线图](formal-verification.md)：现有的形式化锚点与阶段性证明目标。
...
- Psy/DPN、Aleo Leo 和 Cloudflare Workers 从可移植 IR fixture 发射目标源代码；有关每个门控的工具先决条件，请参阅 [validation-gates.md](validation-gates.md)。
