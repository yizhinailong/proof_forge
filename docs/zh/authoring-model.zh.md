...
portable `.learn` 示例，生成一个小型 source AST，并将该 AST 降低到
`contract_source` 共用的 `ContractSpec`/portable IR 边界。
...
1. 将 Learn parser 从当前 portable scalar/event 子集扩展到通过 typed SDK form
   表达 Solana target-extension declaration。
