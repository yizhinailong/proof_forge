...
这些能力记录已知的跨目标语义分歧：能力门会暴露这些分歧，但尚不会逐节点强制它们。
当一个 portable contract 在不同目标上表现不一致时，应先查看这里。
...
参见 [Kaspa Toccata 目标](../targets/kaspa-toccata.md)。
...
参见 [Stellar Soroban 目标](../targets/stellar-soroban.md)。
...
参见 [Internet Computer 目标](../targets/internet-computer.md)。
...
参见 [Algorand AVM 目标](../targets/algorand-avm.md)。
...
参见 [Cardano Plutus/Aiken 目标](../targets/cardano-plutus-aiken.md)。
...
参见 [Tezos Michelson/LIGO 目标](../targets/tezos-michelson-ligo.md)。
...
参见 [Starknet Cairo 目标](../targets/starknet-cairo.md)。
...
参见 [Aleo Leo 目标](../targets/aleo-leo.md) 和
[docs/zh/superpowers/specs/2026-07-01-aleo-leo-design.zh.md](../superpowers/specs/2026-07-01-aleo-leo-design.md)。
...
以下能力仍作为候选，直到 private records、transitions、proofs、imports、deployment
或 devnet validation 被纳入范围。
...
参见 [TON TVM 目标](../targets/ton-tvm.md)。
...
参见 [Bitcoin Script/Miniscript 目标](../targets/bitcoin-script-miniscript.md)。
...
参见 [Zcash Shielded 目标](../targets/zcash-shielded.md)。
...
参见 [Bitcoin Cash CashScript 目标](../targets/bitcoin-cash-cashscript.md)。
...
目前通过 `ProofForge.Evm` / `Lean.Evm` 实现 —— 参见 [targets/evm.md](targets/evm.zh.md)。
