...
| NEAR 实际 lowering | `.add` → `i64.add`（静默回绕），恒为 wrapping | `Backend/WasmHost/LoweringEnv.lean:40` |
...
| NEAR `.add` → `i64.add`（wrapping） | `ProofForge/Backend/WasmHost/LoweringEnv.lean:40` |
