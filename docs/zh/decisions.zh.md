...
| D-025 | 2026-07-01 | 增加 **`solana-sbpf-asm`** 作为新的 Solana 路线（direct sBPF assembly codegen）并进入探索；保留 `solana-sbpf-linker` 作为 fallback | 从 portable IR 直接生成 sBPF assembly 可以避开完整 Lean Zig runtime 链接风险；blueshift-gg/sbpf 工具链负责 assembly 和 linking。见 [RFC 0005](rfcs/0005-solana-sbpf-assembly-backend.md) 与 [设计文档](targets/solana-sbpf-asm.md)。 |
