...
| D-050 | 2026-07-09 | **IR 保持链中立；storage binding 由 target 解析。** Portable IR `StateDecl` 只描述形状（`scalar`/`map`/`array` + type），作者不声明 object/resource/slot。每个 `--target` 通过 `ProofForge.Target.StorageBinding` 映射 portable state。EVM 命名字段已改为中立 metadata（`paramAbiWords`、`proxyPattern?`）；`ProofForge.IR.Portability` 分类 family-only constructor，而不把 binding 放入 portable IR | D-028 要求链中立 authoring，D-027 要求链原生细节留在 adapter/extension。object 与 resource 是 Move target 的 lowering 选择，不是 portable IR annotation。Gate：`just ir-portability-smoke`。 |
| D-051 | 2026-07-10 | **EVM storage packing 采用连续 small-scalar packing，而非每字段一槽。** `ProofForge.Backend.Evm.Plan.storageLayout` 将相邻的 bool/u8/u32/u64/u128/address 标量装入共享 32-byte slot。runtime read/write 与 constructor initialization 使用 Solidity 兼容的低位 offset（`shift = byteOffset * 8`）；写入前按字段宽度 mask，checked 直接写入和复合赋值都会拒绝窄字段溢出，wrapping 写入截断且不污染邻居。hash 与非标量仍独占 slot；NEAR/WasmHost 使用独立的 `__pf_pack_*` key packing | 证据：`Tests/Backend/Evm/EvmPackedStorage.lean`、`scripts/evm/packed-storage-ir-smoke.sh` 与 artifact `storageLayout`。仅在产品需要跨非相邻声明等 Solidity-incompatible packing 或 gas-waste lint 时重审。 |
