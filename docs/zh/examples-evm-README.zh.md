...
- `Counter.lean` 使用 `Storage.load`/`store` 实现了一个简单的计数器，包含 `get`/`set`/`increment`/`decrement` 方法。
...
这通过 `proof-forge --evm-bytecode` (Lean -> EmitYul -> Yul -> `solc --strict-assembly` -> 字节码) 将每个 `.lean` 合约编译为 EVM 字节码。
它在 `PATH` 上需要 Foundry (`cast`/`forge`) 和 `solc`。
...
这会编译示例，并使用 Foundry 的 `vm.etch` cheatcode 对生成的运行时字节码运行 Forge 测试。

## 当前 EVM 支持

目前的支持足以通过 `proof-forge --evm-bytecode` 编写和部署小型 Lean EVM 合约：
...
- 算术：Nat 加/减/乘/除/取模、比较、位运算（全部限制在 U256 范围内）。
...
- `Nat` 限制在 U256 范围内（溢出时回退）；EVM 上没有大数 (bignum) / GMP 支持。
