...
- 已经跑通。
- 工具成熟，Foundry 可以做 smoke test。
- 市场最大，最容易展示 demo。

目标：

- 把现有 EVM backend 从实验状态整理成稳定 baseline。
- 让它成为所有后续 target 的对照组。
...
- NEAR、CosmWasm、Polkadot/ink! 都和 Wasm 有关。
- Wasm 是比较通用的编译目标，长期收益大。

注意：

- 不能把 “Wasm” 当成一个链。Wasm 只是执行格式；每条链的 host ABI 才是关键。
...
建议用三个 milestone 验证愿景，而不是一开始追求完整平台。

### Milestone 1：EVM baseline 产品化

验收标准：

- `proof-forge build --target evm` 形态确定。
- EVM examples、Foundry smoke、CI 全稳定。
- 生成 artifact metadata：bytecode、ABI/methods、proof status、target info。

### Milestone 2：Portable core + shared scenario

验收标准：

- 定义 portable contract IR。
