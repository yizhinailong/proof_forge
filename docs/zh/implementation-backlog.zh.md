...
- EVM 语义计划迁移 TODO：
  ...
  - 已补充：active 聚合 crosscall return assignment 现在进入 `ToYul.crosscallAggregateReturnAssignmentExpandedPlanStatement`，由 ToYul 负责 target/method/call-value 降级、已展开 planned argument word 遍历、helper 名选择、参数顺序以及多返回 assignment 组装；provider-backed `ToYul.crosscallAggregateReturnAssignmentPlanStatement` 只保留给 direct/legacy local 和 storage word-source surface。
  ...
  - 已补充：active expression crosscall 现在通过 `ToYul.crosscallExpandedArgWordPlanExprs` 与 `ToYul.crosscallExpandedExprPlanExpr` 消费已展开的 `.expr` word plans；provider-backed `ToYul.crosscallArgWordPlanExprs` 与 `ToYul.crosscallExprPlanExpr` 只保留给 direct/legacy source-plan surface。
