...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：`storageStructFieldWrite` 和 `storageArrayStructFieldWrite` 的 value 降级现在会对受支持的标量字段值消费同一个 `ExprPlan -> ToYul` 表达式边界。结构体字段 slot 组装仍留在现有 direct slot / `StorageSlotPlan -> ToYul` 边界上。
