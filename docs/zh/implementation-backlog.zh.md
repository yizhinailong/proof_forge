...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：expression 位置的 `storagePathRead` 组装现在会消费 `Lower.buildEffectPlan` 产出的 planned typed `StorageSlotExprPlan` target。direct map、嵌套 map、array、struct-field 和 struct-array-field storage-path read 都会通过 `ToYul.storagePathReadExprFromExprPlan` 负责最终的 `sload` slot 表达式，不再只在兼容外观末端重新计算 slot。active path segment 表达式现在会通过 `Lower.buildStoragePathPlan` 变成携带 `ExprPlan` 的 `StoragePathPlanSegment`；旧 `ValuePlan` slot helper 只保留给直接兼容/fallback surface。
