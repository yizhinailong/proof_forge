...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 `return` 的 statement 组装现在会对受支持的单字 `U32`/`U64`/`Bool`/`Hash`/`Address` 返回值消费一条窄 `StmtPlan -> ToYul` helper，并覆盖分支内返回需要追加 Yul `leave` 的情况。本地 fixed-array 和 struct 聚合返回现在会走 `Lower.returnValueWordPlan? -> ReturnValueWordPlan -> ToYul` 来组装返回 ABI word assignment。bytes/string 返回、literal/storage 聚合返回，以及聚合跨合约调用返回 helper 仍留在现有兼容路径上，直到对应迁移切片补齐更完整的计划级覆盖。
  ...
  - 已开始：direct `storageMapInsert`/`storageMapSet` 写入组装现在会对受支持的标量 map key/value 表达式消费 `Lower.buildEffectPlan` 产出的 `MapWriteTargetPlan` 变体。statement 位置写入和 expression 位置的返回旧值写入现在都会通过 direct `EffectPlan -> ToYul` / `ExprPlan -> ToYul` helper 消费 planned map root slot，不再在兼容外观里做 late lookup。`storageMapContains` 和 `storageMapGet` 读仍使用现有 state-id 路径；storage-path map 写入继续走独立的 `StoragePathWriteTargetPlan` surface，直到 typed map path-expression planning 被拓宽。
  - 已开始：direct `storageArrayWrite` 组装现在会对受支持的标量 index/value 表达式消费 `Lower.buildEffectPlan` 产出的 `ArrayWriteTargetPlan` 变体。该 plan 会携带数组 root slot 和 length，direct `EffectPlan -> ToYul` helper 现在负责最终的 `__proof_forge_array_slot(root, length, index)` 组装，不再通过兼容外观 callback 做 late lookup。`storageArrayRead`、struct-array field 写入，以及 storage-path array 写入仍留在现有 helper/target surface 上，直到它们的元数据被拓宽成 explicit semantic-plan node。
  - 已开始：statement 位置的 `storageStructFieldWrite` 和 `storageArrayStructFieldWrite` 组装现在会对受支持的标量字段 value，以及 struct-array 字段写入中的受支持标量 index，消费一条窄 `StmtPlan.effect` / `EffectPlan -> ToYul` helper。direct struct field slot 查询和 struct-array field slot 元数据仍留在兼容外观中，直到更完整的 storage-path 和 whole-struct 抽取切片补齐覆盖。
  ...
  - 已开始：标量 `ifElse` 和 `boundedFor` 的 control-flow frame 组装现在会消费窄 `StmtPlan -> ToYul` helper。if 条件和合成的 bounded-loop 守卫会消费 `ExprPlan -> ToYul`；受支持的分支/循环 body 语句现在会递归消费 planned 标量绑定、标量/local aggregate-scalar 赋值、断言、返回、revert、标量 storage 写入、map 写入以及 map contains/get 读表达式、array 写入与 array 读表达式、struct-field 写入以及 struct/struct-array field 读表达式、storage-path 写入/复合赋值以及 storage-path 读表达式、静态/动态标量本地 fixed-array 读表达式、静态/动态本地 struct-array 字段读表达式、标量非索引/索引事件 emit，以及受支持 body 语句里的标量 crosscall/create helper-call 表达式。语句排序和暂不支持的 body 形状仍留在 `IR.lean` 兼容外观中，直到完整递归的 `StmtPlan -> Yul` 降级被抽出。
  ...
  - 已开始：聚合 crosscall helper-call 组装以及入口多字返回 assignment 现在位于 `ToYul` 后面。`IR.lean` 仍负责标量 expression fallback 路径里的返回类型检查和聚合参数字展开，但聚合 crosscall return assignment 的判断现在来自 `Lower.aggregateCrosscallReturnAssignmentPlan?`。该 plan 会记录 call mode、target/method/call-value expression plans、已规划的 crosscall argument words，以及 `ReturnPlan` 的 local-name/word-layout 数据；`IR.lean` 消费这些已规划的 `ExprPlan`，并把最终 helper-call 函数名选择、参数顺序以及多返回 Yul assignment 构造委托给 `ToYul`。入口返回里的本地聚合 ABI word 展开现在会通过 `Lower.returnValueWordPlan?` 校验 source local 与期望类型，把 planned `ExprPlan.localAbiWords` source 和 `ReturnPlan` layout 记录到 `ReturnValueWordPlan`，并把最终多字 assignment frame 委托给 `ToYul.returnValueWordPlanAssignments`。相关兼容路径在被表达进语义计划之前仍会直接调用 `ToYul.localAbiWords`。本地聚合 crosscall argument word 展开现在也会把最终 local identifier word 构造委托给 `ToYul.localCrosscallWords`；`IR.lean` 仍负责 storage scalar struct read 以外的其它非 literal 聚合 source，直到这些内容能直接表达在语义计划里。`Lower` 现在会把本地聚合 typed/value/static/delegate crosscall 参数表示为 `ExprPlan.localCrosscallWords`，把 storage scalar struct read 表示为 `ExprPlan.storageCrosscallWords`，并会把 struct literal 与 fixed-array literal crosscall 参数展开为标量 word `ExprPlan`；`IR.lowerExprPlanExpr` 会在选择 helper-call arity 之前消费这些已规划的 word。
