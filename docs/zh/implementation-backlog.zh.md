...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：直接标量 local 赋值和复合赋值的 statement 组装现在总是先消费 `Lower.buildExprPlan`，再调用 `ToYul.scalarAssignmentStmtPlanStatements`。静态本地 fixed-array element 赋值、静态本地 struct-field 赋值，以及静态本地 struct-array field 赋值 target 也会通过 `ExprPlan.localArrayGet` 和 `ExprPlan.structField` 使用同一条 `StmtPlan.assign`/`StmtPlan.assignOp -> ToYul` helper，RHS 通过 `lowerAssignmentValueExpr` 降级。标量 local 的 `lowerExpr` assignment fallback 已删除。`lowerAssignStmt`/`lowerAssignOpStmt` 中的静态聚合 assignment target 现在总是消费 `lowerStaticAggregateScalarAssignmentStmt`（`buildStaticAggregateScalarTargetPlan? -> StmtPlan.assign/assignOp -> ToYul`）；基于 `lowerAssignTargetName` + `lowerAssignmentValueExpr` 的 fallback 已删除，不支持的 target 会显式失败。整聚合赋值、动态聚合 helper snapshot，以及非标量 storage effect 写入仍留在现有兼容路径上，直到对应迁移切片补齐覆盖。整本地 struct 赋值现在总是消费 `Lower.structAssignmentSourcePlans -> ToYul.wholeStructAssignStmtFromPlan`；用 `lowerExpr` fallback 重复 planned source 展开的 IR-local `lowerStructAssignmentSourceExprs` 兼容 helper 已删除。整聚合 fixed-array 和 struct-array 赋值早已只走 planned source 路径。
