...
- 已完成：扩展 EVM IR local fixed-array lowering 到静态嵌套 scalar array，覆盖不可变 read、可变 leaf 赋值、数字 leaf 复合赋值、嵌套 whole-local assignment，以及 RHS 快照，并用 `EvmArrayValueProbe` 跑通 golden Yul、metadata entrypoint、solc bytecode 和 Foundry 运行时验证。动态嵌套 index 和非 scalar 嵌套 leaf 仍保持显式诊断。
