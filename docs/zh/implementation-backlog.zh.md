...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 expression 位置的 crosscall helper-call 组装以及 create/create2 helper-call 组装现在位于 `ToYul` 后面。标量 `call`、带值 `call`、native value transfer、`staticcall`、`delegatecall`、`create` 和 `create2` 的 `ExprPlan` 节点可以直接使用与 helper body 发射相同的 helper 名称选择逻辑降级为 helper call。planned scalar return frame 现在会调用与 expression lowering 相同的 `lowerExprPlanExpr` callback，因此 local/storage aggregate crosscall argument source plan 可以先由 provider 展开，再由 `ToYul` 选择 helper-call 名称和参数顺序。兼容性的 `IR.lean` 表达式降级仍负责 type-env 验证以及 fallback aggregate crosscall argument word expansion。
