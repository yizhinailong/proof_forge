...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：整体本地聚合赋值的 snapshot block 现在位于 `ToYul` 后面。本地 fixed-array 整体赋值 source 现在会先进入 `Lower.fixedArrayAssignmentSourcePlans`，因此本地数组 source 和数组字面量元素表达式会先表示为 `ExprPlan`，再由 `ToYul.wholeFixedArrayAssignStmtFromPlan` 发射 snapshot block。`IR.lean` 仍负责验证并展开嵌套 fixed-array、struct-array 和 struct 赋值 source，但最终的临时变量声明、目标 local 名称以及 assignment block 构造会委托给 `ToYul` helper，使兼容外观不再拥有最终 Yul statement frame。
