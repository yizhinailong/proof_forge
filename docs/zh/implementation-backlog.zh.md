...
- EVM 语义计划迁移 TODO：
  ...
  - 已补充：聚合 crosscall return assignment 现在也会进入 `ToYul.crosscallAggregateReturnAssignmentPlanStatement`，由 ToYul 负责 target/method/call-value 降级、planned argument word 遍历、helper 名选择、参数顺序以及多返回 assignment 组装；兼容 provider callback 只保留给 direct/legacy local 和 storage word-source surface。
