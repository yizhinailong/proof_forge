...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：聚合 crosscall helper-call 组装以及入口多字返回 assignment 现在位于 `ToYul` 后面。expression 位置的聚合 crosscall return 诊断现在来自 `Lower.buildExpressionExprPlan`，聚合 crosscall return assignment 的判断现在来自 `Lower.aggregateCrosscallReturnAssignmentPlan?`。该 plan 会记录 call mode、target/method/call-value expression plans、已规划的 crosscall argument words，以及 `ReturnPlan` 的 local-name/word-layout 数据；`IR.lean` 消费这些已规划的 `ExprPlan`，并把最终 helper-call 函数名选择、参数顺序以及多返回 Yul assignment 构造委托给 `ToYul`。入口返回里的本地聚合 ABI word 展开现在会通过 `Lower.returnValueWordPlan?` 校验 source local 与期望类型，把 planned `ExprPlan.localAbiWords` source 和 `ReturnPlan` layout 记录到 `ReturnValueWordPlan`，并把最终多字 assignment frame 委托给 `ToYul.returnValueWordPlanAssignments`。相关 local ABI 兼容路径仍会把 `ToYul.localAbiWords` 作为最终 emission surface，但 local/type 校验和 struct-field 发现现在会先通过 `Lower.validateLocalAbiWordPlan`、`Lower.localAbiStructFieldIds` 与 `Lower.localAbiStructFields` 再进入该边界。return、indexed event 和 event data 的 storage-backed ABI word provider 现在会通过 `Lower.storageAbiWordPlans`，并先降为显式 `ExprPlan.storageLoad` word plans 再进入最终 Yul 发射。本地聚合 crosscall argument word 展开现在也会把最终 local identifier word 构造委托给 `ToYul.localCrosscallWords`；local provider 的校验和 struct-field 发现现在会通过 `Lower.validateLocalCrosscallWordPlan` 与 `Lower.localCrosscallStructFieldIds`。`IR.lean` 仍负责 storage scalar struct read 以外的其它非 literal 聚合 source，直到这些内容能直接表达在语义计划里。`Lower` 现在会把本地聚合 typed/value/static/delegate crosscall 参数表示为 `ExprPlan.localCrosscallWords`，通过 `Lower.storageCrosscallWordPlans` 把 storage scalar struct read 展开成显式的 `ExprPlan.storageLoad` word plans，并会把 struct literal 与 fixed-array literal crosscall 参数展开为标量 word `ExprPlan`；`IR.lowerExprPlanExpr` 会在选择 helper-call arity 之前消费这些已规划的 word。已规划 crosscall argument word group 的最终遍历和拼接现在会使用 `ToYul.crosscallArgWordPlanExprs`；`IR.lean` 仍提供兼容 `ExprPlan.localCrosscallWords`/`ExprPlan.storageCrosscallWords` 输入所需的 ToYul provider callback，但 active Lower 产出的 storage-backed crosscall 参数不再依赖 IR-local storage provider expansion。标量 expression fallback crosscall lowering 现在也会先调用 `Lower.buildCrosscallArgWordPlansMany` 再进入该 ToYul 边界，旧的 IR-local `lowerCrosscall*ArgWords` 展开树已经删除。
