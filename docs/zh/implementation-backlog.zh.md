...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：dynamic-array push/pop 的成功路径现在会消费携带 root slot 的 `DynamicArrayTargetPlan` target variant，再调用 `ToYul.dynamicArrayPushTargetEffectStmtPlanStatements` / `ToYul.dynamicArrayPopTargetEffectStmtPlanStatements`。因此 root-slot 选择和最终 dynamic-array slot helper 组装不再通过 `IR.lean` callback 重建；旧 callback helper 只保留给 legacy/fallback surface。
