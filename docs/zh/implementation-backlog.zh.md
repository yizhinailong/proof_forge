...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 `storageScalarWrite` 和 `storageScalarAssignOp` 的 statement 组装现在会对受支持的标量 value 表达式消费一条窄 `StmtPlan.effect` / `EffectPlan -> ToYul` helper。state layout 的 slot 解析和 struct storage 写入仍留在兼容外观中，直到对应迁移切片补齐覆盖。
