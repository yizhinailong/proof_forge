...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：direct `storageStructFieldWrite` 组装现在会对受支持的标量 field value 消费 `Lower.buildEffectPlan` 产出的 `StructFieldWriteTargetPlan` 变体。该 plan 会携带 struct field storage slot，direct `EffectPlan -> ToYul` helper 现在负责最终的 `sstore(fieldSlot, value)` 组装，不再通过兼容外观 callback 做 late lookup。`storageArrayStructFieldWrite` 仍使用现有 struct-array slot callback；struct/struct-array field 读仍留在现有 state-id/field lookup 路径上，直到对应 target 切片补齐覆盖。
