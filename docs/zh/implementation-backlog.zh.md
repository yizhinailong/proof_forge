...
- FV-4：已在 `Backend/Evm/Refinement.lean` 中落地初始 EVM Counter 可执行追踪义务，并由 `Backend/Evm/YulSemantics.lean` 支撑。它镜像 `Backend/WasmNear/Refinement.lean` 的 scalar IR trace，检查 selector-dispatched Yul surface，并执行 Counter 形态的已发射 Yul 子集（`sstore`、`sload`、标量算术、`mstore`、`return`），将可观察 EVM return words 与 IR trace 对比。下一步：将解释器和义务扩展到 Counter 之外，优先面向 ValueVault 和更广的标量/存储 IR 形态；在解释器存在之前，让 Psy/Solana 保持在差异门控上。
...
- Solana Pinocchio CI equivalence（工作流 7）：source/reference equivalence
  suite 已纳入 `just solana-light`；剩余工作是通过可靠安装 Solana
...
- Gate P0（主三链签署）：Gate G0 加上 D-045 中 Solana、Ethereum/EVM 和 NEAR/Wasm
  的剩余生产级硬化。
...
- **工作流 29 — CLI 产品界面。** 针对 `proof-forge build|emit|check --target <id> --fixture <id>` 的 RFC，折叠约 136 种发射模式; 遗留标志在一个版本中成为别名。必须在 testkit M4 绑定到当前标志之前完成规划。
...
2. **Gate P0（主三链完成规约，D-045）：** 按实现优先级完成
   ...
   行为/资源预算切片；Gate P0 还包含 Solana Pinocchio CI equivalence、EVM
   semantic-plan migration，以及 NEAR/Wasm 的本地执行、制品、诊断和 CI 签署。
   Gate 关闭前，已落地的 Aptos/CosmWasm spike 冻结在 M1/M2。逐项状态见
   [gate-status.md](gate-status.md)。
