...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：selector-dispatch case 组装现在会消费 `ToYul` 里的 `EntrypointPlan` surface helper。ABI validation/decode 语句、function-call 参数组装、return-data encoding、dynamic-param free-memory-pointer 设置，以及 proxy fallback 选择仍留在 `IR.lean` 兼容外观中，直到更完整的 dispatch 降级移动到 `EntrypointPlan -> Yul` 后面。
