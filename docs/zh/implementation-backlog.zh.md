...
- [x] System Program transfer CPI packing 骨架现在会生成 `sol_invoke_signed_c` 所需的 C ABI 形状：program id bytes、C `SolAccountMeta[]`、`system.transfer` instruction data（`u32` discriminator + `u64` lamports placeholder）、C `SolInstruction`、绑定到生成的 multi-account input layout 的 `SolAccountInfo[]`、signer seed table，以及 syscall register setup。覆盖：`Tests/SolanaCpiPacking.lean`、`Tests/SolanaSdkManifest.lean`、`scripts/solana/sdk-smoke.sh`。
...
- System Program CPI runtime completion：把 lamports source 降级进 instruction data，补 `system.create_account` packing，再通过 Web3.js 验证余额和 owner。
