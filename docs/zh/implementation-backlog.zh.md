...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：最终事件 block 组装现在会消费 `EventPlan -> ToYul` helper，用于签名 topic 设置、索引 topic 语句、非索引数据存储，以及最终 `log1`-`log4` 语句选择。事件字段值求值仍会向字段 word-plan provider 请求由 `Lower` 拥有的 `ExprPlan` word 序列，直到数据字和索引 topic 表达式组装完整移动到 `EventPlan -> Yul` 后面。planned scalar-body event effect 现在会通过 `ToYul.eventEffectStmtPlanStatements`，因此 `StmtPlan.effect` 会在 ToYul 后面选择 `eventEmit`/`eventEmitIndexed` block 构造，并且 ToYul 会拥有字段/值数量检查、word-plan-to-Yul 表达式降级以及 indexed-topic/data 路由，而 IR 只提供字段 word plan。早期的 Yul-expression callback helper 形状已经移除。
