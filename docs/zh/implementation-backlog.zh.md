...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：聚合 crosscall helper-call 组装以及入口多字返回 assignment 现在位于 `ToYul` 后面。`IR.lean` 仍负责返回类型检查、ABI 返回名查询和聚合参数字展开，但聚合 helper-call 函数名选择、参数顺序以及最终多返回 Yul assignment 构造会委托给 `ToYul`。入口返回和相关兼容路径里的本地聚合 ABI word 展开现在也会把最终 local identifier / data-pointer word 构造委托给 `ToYul.localAbiWords`；`IR.lean` 在调用前仍负责校验 source local、期望类型和 struct 字段可用性。本地聚合 crosscall argument word 展开现在也会把最终 local identifier word 构造委托给 `ToYul.localCrosscallWords`；`IR.lean` 仍负责 crosscall 参数形状校验和非本地聚合 source，直到这些内容能直接表达在语义计划里。
