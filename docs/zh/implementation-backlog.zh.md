...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：statement 位置的 `storagePathWrite` 和 `storagePathAssignOp` 组装现在会消费 `Lower.buildEffectPlan` 产出的 planned `StoragePathWriteExprTargetPlan` 变体，并在 direct `mapKey`、`index`、`field`、`index`+`field` 以及连续嵌套 `mapKey` 路径中，对受支持的标量写入/复合赋值 RHS value 使用 direct `EffectPlan -> ToYul` helper。兼容性的 `lowerStoragePathWriteTarget` helper 现在也会先构造 `Lower.buildStoragePathPlan`，再通过 `StoragePathWriteExprTargetPlan -> ToYul` 降级，因此 raw path segment 表达式不再通过 `ValuePlan` wrapper 进入这个 helper。剩余 storage-path 抽取工作是等 direct 调用方和 diagnostic-only fallback path 不再需要旧 callback surface 后删除它们。
  ...
  - 已开始：storage-path write fallback frame 现在也会复用 `ToYul.storagePathWriteTargetStatements`；`IR.lean` 仍负责选择 fallback target 和 scalar value，但不再手写 map write helper、single-slot `sstore` 或 nested-map value/presence writeback block。
  - 已开始：旧的 `lowerMapPathWriteStmt` nested-map writeback helper 已在 storage-path write fallback 迁移到共享 `ToYul` frame 后移除。
  - 已开始：storage-path assign-op fallback 的 writeback frame 现在也会复用 `ToYul.storagePathAssignOpTargetStatements`；`IR.lean` 仍负责选择 fallback target 和 scalar value，但不再手写 map helper、single-slot `_slot` block 或 nested-map value/presence writeback block。
