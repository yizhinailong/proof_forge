...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 `return` 的 statement 组装现在会对受支持的单字 `U32`/`U64`/`Bool`/`Hash`/`Address` 返回值消费一条窄 `StmtPlan -> ToYul` helper，并覆盖分支内返回需要追加 Yul `leave` 的情况。本地、literal，以及 storage-backed fixed-array/struct 聚合返回现在会走 `Lower.returnValueWordPlan? -> ReturnValueWordPlan -> ToYul` 来组装返回 ABI word assignment。bytes/string 返回以及聚合跨合约调用返回 helper 仍留在现有兼容路径上，直到对应迁移切片补齐更完整的计划级覆盖。
