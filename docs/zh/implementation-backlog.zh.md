...
- 已完成：扩展 `scripts/portable/token-intent-smoke.sh`，使 `Examples/Shared/FungibleToken.lean`、`Examples/Shared/FeeToken.lean` 和遗留 `.learn` 输入路径都复用 Lean `TokenSpec` 计划，发射结构化 Solana SPL Token / Token-2022 plan JSON，并使用 Rust `token_plan_smoke` harness 离线验证计划。
...
- 已完成：添加 `ProofForge.Contract.Token.Examples.SoulboundToken`、`Tests/TokenPlanEmit.lean`、`scripts/solana/token-2022-non-transferable-live-smoke.sh` 以及在 Surfpool 上基于 TokenSpec 的 `just solana-token-2022-non-transferable-live`，用 Rust harness 执行 Lean `.lean` Token-2022 不可转账计划。旧的 `solana-token-2022-non-transferable-web3` 入口保留为兼容 alias。实时运行器初始化 `NonTransferable`，创建 Token-2022 关联代币账户，铸造初始供应量，验证 mint/账户扩展，证明 `TransferChecked` 被拒绝，然后销毁代币并通过 Rust RPC 账户读取验证余额和供应量。
