...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：checked-arithmetic helper 的需求现在会在 `Lower.buildFullModulePlan` 和 `Lower.buildFullModulePlanWithTargetPlan` 中，从已经构造好的 `EntrypointPlan.body` `StmtPlan`/`ExprPlan` tree 发现，包括 planned `.checkedArith` 表达式以及 planned `assignOp`/storage assign-op 节点。`IR.buildSemanticPlan` 会保留 Lower 拥有的 `usesCheckedArithmetic` 字段，而不是在 plan 构造后重新扫描模块。不完整 plan 的 fallback lowering 仍会调用 `Validate.moduleUsesCheckedArithmetic`。本地 fixed-array getter helper 的需求仍由 `Lower.buildLocalArrayGetLengths` 和 `Lower.buildNestedLocalArrayGetShapes` 发现，等待后续切片再迁到 planned body surface。
