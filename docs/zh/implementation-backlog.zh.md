...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 `storageScalarWrite` 和 `storageScalarAssignOp` 的 value 降级现在也会对受支持的标量 RHS 表达式消费同一个 `ExprPlan -> ToYul` 表达式边界。结构体 storage 写入、array 写入、struct-field 写入和 storage-path 写入仍留在兼容路径上，直到对应迁移切片补齐覆盖。
