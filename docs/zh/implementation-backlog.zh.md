...
- 已完成：扩展 `scripts/portable/token-intent-smoke.sh`，使 `Examples/Shared/FungibleToken.lean`、`Examples/Shared/FeeToken.lean`、`Examples/Shared/SoulboundToken.lean` 和遗留 `.learn` 输入路径都复用 Lean `TokenSpec` 计划，发射结构化 Solana SPL Token / Token-2022 plan JSON，并使用 Rust `token_plan_smoke` harness 离线验证计划。
- 已完成：添加 `scripts/solana/token-plan-live-smoke.sh` / `just solana-token-plan-live` 以在 Surfpool 上用 Rust harness 执行来自 `Examples/Shared/FungibleToken.lean` 的结构化 SPL Token 计划。旧的 `solana-token-plan-web3` 入口保留为兼容 alias。实时运行器创建 mint 和关联代币账户，铸造初始供应量，执行计划的 `mint_to`、`transfer_checked`、`approve`、`burn`、`revoke` 和 mint-authority `set_authority` 操作，并通过 Rust RPC 账户读取验证余额、供应量、代理状态和权限撤销。
- 已完成：添加 `scripts/solana/token-2022-transfer-fee-live-smoke.sh` / `just solana-token-2022-transfer-fee-live`，用 Rust harness 在 Surfpool 上执行来自 `Examples/Shared/FeeToken.lean` 的结构化 Token-2022 转账费用计划。旧的 `solana-token-2022-transfer-fee-web3` 入口保留为兼容 alias。实时运行器初始化 `TransferFeeConfig`，创建 Token-2022 关联代币账户，铸造初始供应量，执行 `TransferCheckedWithFee`，验证源余额、接收者净余额和接收者预留费用，直接从 token 账户提取预留费用，然后运行第二次转账，将预留费用归集到 mint，从 mint 提取费用，并通过 Rust RPC 账户读取验证费用接收者余额以及已清除的账户/mint 预留金额。
- 已完成：添加 `Examples/Shared/SoulboundToken.lean`、`scripts/solana/token-2022-non-transferable-live-smoke.sh` 以及在 Surfpool 上基于 TokenSpec 的 `just solana-token-2022-non-transferable-live`，通过 target-first `proof-forge build --token` 路径用 Rust harness 执行 shared Lean Token-2022 不可转账计划。旧的 `ProofForge.Contract.Token.Examples.SoulboundToken` import 路径和 `solana-token-2022-non-transferable-web3` 入口保留为兼容 alias。实时运行器初始化 `NonTransferable`，创建 Token-2022 关联代币账户，铸造初始供应量，验证 mint/账户扩展，证明 `TransferChecked` 被拒绝，然后销毁代币并通过 Rust RPC 账户读取验证余额和供应量。
