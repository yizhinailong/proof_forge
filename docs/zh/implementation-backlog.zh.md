...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：整体结构体 `storageScalarWrite` 的 struct-literal 字段 value 降级现在会对受支持的标量字段表达式消费同一个 `ExprPlan -> ToYul` 表达式边界，同时保留 storage 写入前的字段临时变量 snapshot。local struct source、storage-struct read source，以及 statement-level storage-struct write 组装仍留在兼容外观中，直到完整 `StmtPlan -> Yul` 抽取。
  - 已开始：statement 位置的 `storagePathWrite` 组装现在会在 direct `mapKey`、`index`、`field`、`index`+`field` 以及连续嵌套 `mapKey` 路径中，对受支持的标量写入 value 消费一条窄 `StmtPlan.effect` / `EffectPlan -> ToYul` helper。路径 slot 计算和路径形状诊断仍留在 `IR.lean` 兼容外观中。`storagePathAssignOp` 的 value 降级仍会对受支持的标量 map key 和复合赋值 RHS 表达式消费现有 `ExprPlan -> ToYul` 表达式边界；它的 statement 组装是下一步 storage-path 抽取切片。
