...
- EVM 语义计划迁移 TODO：
  ...
  - 已补充：聚合 crosscall return assignment 现在也会进入 `ToYul.crosscallAggregateReturnAssignmentPlanStatement`，由 ToYul 负责 target/method/call-value 降级、planned argument word 遍历、helper 名选择、参数顺序以及多返回 assignment 组装；`IR.lean` 只提供 local 和 legacy storage word source provider callback。
  - 已补充：storage-backed aggregate struct crosscall argument 现在会在 `Lower` 中展开成显式的 `CrosscallArgWordPlan.expr (.storageLoad ...)` word plans；active path 不再保留 provider-backed 的 storage source marker。
