...
- 已完成：加入 EVM IR 扁平 struct local fixed array，按 element field 展开为确定性 Yul local，支持静态和动态 `field(arrayGet(localArray, index), name)` 读取、可变字段赋值、数字字段复合赋值、`U64`/`U32`/`Bool`/`Hash` 字段覆盖、动态越界 revert，并用 `EvmStructArrayValueProbe` 跑通 golden Yul、metadata entrypoint/能力校验、solc bytecode、Foundry 运行时验证和 CI 覆盖。
