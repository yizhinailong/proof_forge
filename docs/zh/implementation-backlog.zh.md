...
| R3：尚无端到端证明把用户不变量连接到生成制品 | 部分接受：已有源级证明、FV-2 aggregate/storage/map/control-flow/event-log IR traces、第一批基于 IR 语义的 FV-8 ValueVault accounting/net-value invariant anchors、NEAR trace obligations 加 Counter 和 ValueVault EmitWat artifact-surface/offline-host execution-surface obligations、NEAR ValueVault backend-invariant state bridge、NEAR host import-signature、host-call frame、per-step storage-snapshot、storage-byte 和 log-payload-byte obligations，以及 EVM FV-4 可执行 Yul trace anchors；但完整 IR-to-artifact 语义保持还没有完成。EVM map/storage/aggregate/control-flow/event 切片现在已经把覆盖到的 FV-2 IR traces 接到可执行 Yul obligations。 | 将 NEAR FV-4 从新的 backend-invariant/import/frame/storage-byte/log-payload-byte bridge 继续扩展到更丰富的 Wasm/offline-host 语义边界，再证明超出当前 state/IO、host-ABI、host-call-frame、storage-snapshot、storage-byte 和 log-payload-byte anchors 的语义保持 |
...
3. 继续形式化验证：FV-2 已具备 aggregate/storage executable traces、
   ...
   storage 或 key count。offline-host surface 现在还会固定 ValueVault 事件的
   byte-level `log_utf8` payload hex fragments，把 host payload bytes 和便于阅读的
   log line fragments 分开检查。FV-8 现在也有第一个 ValueVault IR invariant anchor，
   覆盖共享 11 步场景的 return trace、accounting、final storage 和 net-value
   检查。最新的 NEAR FV-4 切片新增了可由 `native_decide` 检查的
   backend-invariant state/import/frame/storage-byte/log-payload-byte bridge：ValueVault offline-host
   ...
   每一步 offline-host storage snapshot 和 storage-byte snapshot 都被固定，并且
   host import signatures 与 host-call frames 会在 WAT 打印前被固定。下一步把
   这条 bridge 从 state/IO、host-ABI、host-call-frame、storage-snapshot 和
   storage-byte、log-payload-byte equality 扩展到更丰富的 Wasm memory/host 语义边界。
