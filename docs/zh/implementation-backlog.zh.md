...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：标量 `storageScalarWrite` 和 `storageScalarAssignOp` 的 statement 组装现在会对非 struct 标量状态消费 `Lower.buildEffectPlan` 产出的 `ScalarStorageTargetPlan` 变体。该 plan 会携带 storage slot 以及 packed byte offset/width，direct `EffectPlan -> ToYul` helper 负责最终 packed write/assign-op frame。整体 struct scalar storage 写入仍留在兼容路径中，直到字段展开能表示为 planned storage target。
  - 已开始：statement 位置的 `storageMapInsert`/`storageMapSet` 写入组装现在会对受支持的标量 map key/value 表达式消费一条窄 `StmtPlan.effect` / `EffectPlan -> ToYul` helper。expression 位置的返回旧值 map 写入仍消费现有 `ExprPlan -> ToYul` 表达式 helper；map root slot 查询和 storage-path map 写入仍留在兼容外观中，直到对应迁移切片补齐覆盖。
  - 已开始：statement 位置的 `storageArrayWrite` 组装现在会对受支持的标量 index 和 value 表达式消费一条窄 `StmtPlan.effect` / `EffectPlan -> ToYul` helper。数组 state 的 root slot/length 查询仍留在兼容外观中，最终数组元素 slot 表达式仍留在现有 array-slot helper 边界上。
