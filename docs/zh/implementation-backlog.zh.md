...
- EVM 语义计划迁移 TODO：
  ...
  - 已补充：local aggregate struct/fixed-array crosscall argument 现在会在 `Lower` 中展开成显式的 `CrosscallArgWordPlan.expr (.local ...)` word plans；storage-backed aggregate struct crosscall argument 也会在 `Lower` 中展开成显式的 `CrosscallArgWordPlan.expr (.storageLoad ...)` word plans；active path 不再保留 provider-backed 的 local 或 storage source marker。
