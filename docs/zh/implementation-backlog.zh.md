...
2. **Gate P0（主三链完成规约，D-045）：** 按实现优先级完成
   ...
   行为/资源预算切片；Gate P0 还包含 Solana Pinocchio CI equivalence、
   已签署的 EVM P0-2 semantic-plan/FV-4 证据，以及 NEAR/Wasm 的本地执行、
   制品、诊断和 CI 签署。Gate 关闭前，已落地的 Aptos/CosmWasm spike 冻结在
   M1/M2。逐项状态见
   [gate-status.md](gate-status.md)。
