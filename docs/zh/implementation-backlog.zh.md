...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：selector-dispatch case 组装现在会消费 `ToYul` 里的 `EntrypointPlan` surface helper，unit/static ABI-word dispatcher return-data encoding 以及 dynamic `bytes`/`string` dispatcher return-data encoding 也会通过同一边界消费 `ReturnPlan`。dispatch-block setup 现在会消费 entrypoint parameter plans，在 selector switch 前为 dynamic ABI 参数初始化 free-memory pointer。ABI validation/decode 语句、function-call 参数组装以及 proxy fallback 选择仍留在 `IR.lean` 兼容外观中，直到更完整的 dispatch 降级移动到 `EntrypointPlan -> Yul` 后面。
