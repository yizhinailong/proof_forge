...
- EVM 语义计划迁移 TODO：
  ...
  - 已开始：最终事件 block 组装现在会消费 `EventPlan -> ToYul` helper，用于签名 topic 设置、索引 topic 语句、非索引数据存储，以及最终 `log1`-`log4` 语句选择。事件字段值求值仍使用 callback 提供的兼容 hook，直到数据字和索引 topic 表达式组装完整移动到 `EventPlan -> Yul` 后面。planned scalar-body event effect 现在会把最终 statement frame 路由到 `ToYul.eventEffectStmtPlanStatements`，因此 `StmtPlan.effect` 会在 ToYul 后面选择 `eventEmit`/`eventEmitIndexed` block 构造，而 IR 只提供字段 word/topic callback。
