...
1. 一种可移植合约 IR，其带有效应的调用是类型化的能力，而非目标操作码。
2. 一个能力命名空间和一张每目标降级表，后端参考该表将每个能力降级为具体的宿主原语。
3. 每个目标的运行时 profile，说明 Lean 语言运行时如何与链宿主运行时协调，且编译器在降级前会进行静态检查。

本 RFC 明确的核心主张是：目标选择是一个构建时、表驱动且经过静态检查的决策。不存在针对链的运行时分派。合约要么在其能力和运行时约束下干净地降级到目标，要么在生成任何制品之前被拒绝并给出精确的诊断。

本 RFC 未定义完整的 IR 表面。IR 单元结构（模块、入口、状态、效应）在 [`portable-ir.md`](../portable-ir.md) 中定义；规范的能力 id 和每目标支持矩阵在 [`capability-registry.md`](../capability-registry.md) 中。本 RFC 仅定义了那些规范未涵盖的核心部分：能力降级规则格式、运行时 profile 和静态检查。除效应/能力表示之外的 IR 类型和语句细节推迟到 `portable-ir.md` 以及在了解 Solana 约束后的实现中。
...
本 RFC 修正了共享层，使得后端仅在降级表和运行时 profile 上有所不同，而不在思维模型上有所不同。

## 非目标

- 本 RFC 不指定 IR 单元结构（模块/入口/状态/效应）——那是 [`portable-ir.md`](../portable-ir.md)。它仅指定后端和能力检查器所依赖的能力降级规则格式和运行时 profile。
...
## 与其他规范的关系ProofForge 现在将共享层设计拆分到多个文档中。本 RFC 是运行时和降级的权威依据；其他文档则负责各自的领域。

| 领域 | 权威依据 | 本 RFC 的角色 |
|---|---|---|
| IR 单元结构 | [`portable-ir.md`](../portable-ir.md) | 使用它；在 `Effect` 之上添加能力降级规则 |
| 能力 id + 支持矩阵 | [`capability-registry.md`](../capability-registry.md) | 使用 id；定义降级规则格式以及后端如何使用它们 |
...
- **D-004/D-005**：规范的 Solana id 是 `solana-sbpf-linker`；Zig 分支保留作为备选/参考。下方的运行时 profile 使用规范 id。
...
可移植 IR 不包含目标操作码。它包含能力调用：即对带有能力 id 注解的 opaque 函数的调用。IR 在调用旁记录能力标签，以便后端可以对其进行降级，而无需从被调用者名称中重新推断。
...
合约通过在每个目标的 SDK 中调用一个声明为 `opaque` 且带有 `@[capability "..."]` 属性的函数来使用一种能力：
...
Lean 前端将此注解保留到 LCNF 中，IR 构建器在每个调用点记录它。后端不再对 `lean_evm_*` 名称进行模式匹配；它们读取能力标签。

### 能力命名空间

能力 id 的规范集合及其逐目标支持矩阵位于 [`capability-registry.md`](../capability-registry.md)，这是 id 的唯一事实来源。RFC 0003 不会重新定义它们；它消费它们。
...
| solana-sbpf-linker | `crosscall.cpi` | `带有 account metas 的 syscall CPI |
...
在降级之前，编译器会计算合约所使用的能力 id 集合（IR 中所有能力调用的并集），并根据目标的受支持集合进行检查。这是一个集合操作；它要么成功，要么产生精确的诊断。
...
| solana-sbpf-linker | 受限运行时（暂定） | 模块 "solana" | 由 spike 待定 | 待定 | 待定 |
...
2. 将 Lean 源代码编译为 LCNF，保留 `@[capability]` 注解并记录特性使用情况（闭包、递归、分配）。
3. 构建可移植 IR：入口、类型、状态转移以及标记有其 `CapId` 的能力调用。证明在步骤 2 中进行验证并擦除；IR 携带的是证明状态元数据，而非证明项。
4. **静态检查（集合运算，构建时，不可协商）：**
   - `usedCapabilities ⊆ supportedCapabilities(target)`，否则拒绝。
   - `usedFeatures ⊆ runtimeProfile.features`，否则拒绝。
5. 降级：对于每个能力调用，查找目标的降级规则并发射宿主原语（操作码 / 桥接调用 / 系统调用 / 生成的 API）。
6. 根据 `mode` 构建运行时：
...
- 已定决策（D-001..D-010，包括并行的阶段 2 spike）：`docs/decisions.md`。
