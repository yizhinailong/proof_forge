...
- **Phase 6b —— 把 `powdr-labs/evm-semantics` 集成为 opt-in EVM refinement target。**
  将 `powdr-labs/evm-semantics` 作为按 commit pin 的 opt-in `lake` 依赖，只供 EVM refinement 模块使用；默认构建继续保持 mathlib-free。提供薄适配器 `ProofForge/Backend/Evm/EvmBytecodeSemantics.lean`，暴露 powdr 的 `State` / `Step` / 可执行 `stepF` 并与 `ObservableStep` 对齐。交付物：可在 Lean 证明中调用的、由外部 conformance baseline 支撑的 EVM 字节码语义。
  **状态（2026-07-07）：首选目标已切换 —— 仅落地 seam。** 早先 `EVMYulLean` 路线因 Lean 工具链 + mathlib 版本不匹配而阻塞（`v4.22.0` vs ProofForge 的 `v4.31.0`）。`require` 项未加入 `lakefile.lean`；`lake build` 保持绿色。当前 seam 已镜像 `powdr-labs/evm-semantics` 的形状：stub `State`、关系式 `Step`、可执行影子 `stepF`、`runBytecode`，以及 `sorry`-free stub 定理（`stepF_sound`、`step_noop`、`runBytecode_empty`）。剩余工作是把 powdr pin 到 opt-in target 后面、让 mathlib 不进入默认 target graph，并用真实 `EvmSemantics` imports 替换 stub body。还需确认 powdr 是否暴露 Yul 级关系；否则 Yul→bytecode 的 `solc` 步骤需保留为显式信任边界。见 [`docs/tier-c-proof-feasibility.md`](../../tier-c-proof-feasibility.md) §2。
...
- 路径 5b：`docs/tier-c-proof-feasibility.md`（新增 —— 本步落地）。
  **2026-07-07 落地（Phase 6a）：** `ProofForge/IR/StepSemantics.lean`、
  ...
  未来：opt-in `powdr-labs/evm-semantics` lake target（6b），以及
  `ProofForge/Backend/Evm/EvmBytecodeSemantics.lean` 中真实 powdr adapter body（6b）。
