...
- [x] 同一个 `contract_source` 模块降级到 EVM + Solana + NEAR（见
      `Examples/Product/Counter.lean` 和 `just portable-counter-multi-target`）。
- [x] ValueVault 从同一个 `contract_source` 模块降级到 EVM + Solana + NEAR，
      覆盖 EVM metadata、Solana manifest/IDL/client metadata，以及 NEAR
      WAT/deploy metadata（见 `Examples/Product/ValueVault.lean` 和
...
legacy `Examples/Backend/Learn/ValueVault.learn` fixture 继续保留，用于 parser
...
| 场景 | 断言指标 | 典型容差 |
|---|---|---|
| Counter | 每步 `evm_gas`、`solana_cu`、`wasmtime_fuel_cumulative` | EVM ±10%，Solana/NEAR ±5% |
| ValueVault | 同上 | 同上 |
...
将报告的 `solana_cu`、`evm_gas`、`wasmtime_fuel_cumulative` 复制到 scenario 文件。详见
