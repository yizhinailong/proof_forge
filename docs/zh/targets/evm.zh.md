...
| `crosscall.invoke` | SDK `call`, `staticcall`, `delegatecall`, `create`, `create2`；portable IR `crosscallInvoke` 降为同步 EVM `call`，method 使用低 32 位 selector，参数是 32-byte word，调用失败和返回不足一个 word 都会 revert；`crosscallInvokeTyped` 增加 Bool/U32/U64/Hash scalar-word 参数和返回，并对 Bool/U32 返回做范围 guard，同时支持 entrypoint 直接返回 normal call 的扁平 struct 和 scalar fixed-array return data；`crosscallInvokeValueTyped` 会把显式 U64 call value 转发到 EVM `call` 的 value slot；`crosscallInvokeStaticTyped` 通过 EVM `staticcall` 降级 typed scalar-word 调用，并保留 static context 下状态写入失败的行为；`crosscallInvokeDelegateTyped` 通过 EVM `delegatecall` 降级 typed scalar-word 调用，并保留 caller storage context |
...
- 生产 EVM SDK 路径仍然通过 LCNF/EmitYul 降级；portable IR EVM 后端目前覆盖标量 storage/ABI、断言、局部赋值、局部复合赋值、标量 storage 复合赋值、条件分支、静态 bounded loop、通过 Yul `leave` 实现的分支/loop 内早退、context read、scalar 和扁平 aggregate event data、`Hash` word 值与 hashing、带托管 key presence 的 word key/value `Map<K, V, N>` storage、`Bool`/`U32`/`U64`/`Hash` 固定 storage array、扁平 scalar storage struct、扁平 struct 固定 storage array、带静态和动态 index 的不可变和可变 local fixed-array value、标量/hash 字段上的扁平不可变和可变 local struct value、带静态/动态字段访问的扁平 struct local fixed array、扁平静态聚合 ABI 参数/返回、同步返回一个 word 的 `crosscallInvoke`、覆盖 `Bool`/`U32`/`U64`/`Hash` 的 typed scalar-word `crosscallInvokeTyped`、normal typed call 的扁平 struct 和 scalar fixed-array entrypoint 直接返回、带 value 的 typed scalar `crosscallInvokeValueTyped`、typed scalar `crosscallInvokeStaticTyped`，以及 typed scalar `crosscallInvokeDelegateTyped`，其他更宽的 portable IR 节点仍以显式诊断拒绝。
- Portable IR EVM 目前仍缺少动态或嵌套聚合 ABI 值、非 word 或 aggregate map 形态、嵌套 array、超出扁平 struct array 的 nested local struct、更完整的 event declaration、合约创建 IR 节点、aggregate 跨调用参数、value/static/delegate typed crosscall 的 aggregate 返回、variable-length 跨调用返回数据，以及真实 creation transaction 或 broadcast manifest。
...
`EvmCrosscallProbe` 验证 portable IR `crosscallInvoke`、`crosscallInvokeTyped`、`crosscallInvokeValueTyped`、`crosscallInvokeStaticTyped` 和 `crosscallInvokeDelegateTyped` 会降为按 arity、返回类型、value 模式、static 模式和 delegate 模式区分的 Yul helper。EVM IR v0 把 target 表达式解释为地址 word，把 method 表达式解释为低 32 位 selector，把 scalar 参数解释为 32-byte ABI word，并把带 value 调用的 call value 解释为 U64 word。helper 会打包 calldata，执行 `call(gas(), target, 0, ...)`、`call(gas(), target, call_value, ...)`、`staticcall(gas(), target, ...)` 或 `delegatecall(gas(), target, ...)`，在调用失败或返回数据短于预期大小时 revert，并解码一个或多个 32-byte 返回 word。Normal typed call helper 覆盖 `Bool`、`U32`、`U64`、`Hash`、entrypoint 直接返回的扁平 struct 和 scalar fixed array；Bool 和 U32 helper 会在返回给 dispatcher 前拒绝越界 return word。带 value、static 和 delegate 的 typed helper 目前仍只覆盖 scalar word。对应 smoke 会检查 golden Yul 可复现、`solc --strict-assembly` 字节码生成、metadata 能力 `crosscall.invoke`、metadata entrypoint、Foundry U64 零/一/二参数调用、typed Bool/U32/Hash 调用、扁平 struct 和 scalar fixed-array aggregate typed return、aggregate Bool/U32 malformed return guard、native value 转发到 payable callee、U64 read-only staticcall 返回、Bool/U32/Hash static typed return、非法 static Bool/U32 return guard、static context 状态写入失败、caller-storage delegatecall 读写、Bool/U32/Hash delegate typed return、非法 delegate Bool/U32 return guard、callee revert、短返回 revert、非法 typed return revert，以及未知 selector revert。
