...
| `crosscall.invoke` | SDK `call`, `staticcall`, `delegatecall`, `create`, `create2`；portable IR `crosscallInvoke` 降为同步 EVM `call`，method 使用低 32 位 selector，参数是 32-byte word，调用失败和返回不足一个 word 都会 revert；typed crosscall 支持 Bool/U32/U64/Hash scalar-word 参数，也支持把扁平 struct 和 scalar fixed-array 参数展开为 ABI word；`crosscallInvokeTyped` 返回 Bool/U32/U64/Hash scalar word 并对 Bool/U32 返回做范围 guard，同时支持 entrypoint 直接返回 normal call 的扁平 struct 和 scalar fixed-array return data；`crosscallInvokeValueTyped` 会把显式 U64 call value 转发到 EVM `call` 的 value slot，并支持同样的 scalar 与 entrypoint 直接 aggregate return；`crosscallInvokeStaticTyped` 通过 EVM `staticcall` 降级 typed 调用，支持 scalar 与 entrypoint 直接 aggregate return，并保留 static context 下状态写入失败的行为；`crosscallInvokeDelegateTyped` 通过 EVM `delegatecall` 降级 typed 调用，支持 scalar 与 entrypoint 直接 aggregate return，并保留 caller storage context |
