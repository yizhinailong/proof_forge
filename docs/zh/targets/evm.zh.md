...
`EventProbe` 验证 portable IR event emission 到 Yul log 的降级。EVM IR v0 会根据 event name 和 field type 生成 Solidity-style event signature，例如 `ValueEvent(uint64)`、`PairEvent((uint64,uint64))`、`StoragePairEvent((uint64,uint64))`、`StorageArrayEvent(uint64[2])`、`ArrayEvent(uint64[2])`、`PairArrayEvent((uint64,uint64)[2])`、`StoragePairArrayEvent((uint64,uint64)[2])`、`IndexedPair((uint64,uint64),uint64)`、`IndexedStoragePair((uint64,uint64),uint64)`、`IndexedTwoValues(uint64,uint64,uint64)`、`IndexedThreeValues(uint64,uint64,uint64,uint64)`、`IndexedStorageArray(uint64[2],uint64)`、`IndexedArray(uint64[2],uint64)`、`IndexedStoragePairArray((uint64,uint64)[2],uint64)` 或 `IndexedPairArray((uint64,uint64)[2],uint64)`，再用它派生 topic0。普通 `eventEmit` 降为 `log1`；`eventEmitIndexed` 会把最多三个 indexed field 快照进 topic，生成 `log2`、`log3` 或 `log4`。scalar indexed field 直接成为 topic；扁平 struct（包括 storage-backed scalar struct read）、scalar fixed array，以及元素为扁平 struct 的 fixed array 会先展开为 ABI-style 32-byte word，再对这些 word 执行 `keccak256` 作为 indexed topic；storage-backed fixed array 也会从 storage array read 和 storage array struct field read 展开后按同样方式 hash。非 indexed data field 可以是 scalar word、来自 local value 或 storage read 的扁平 struct、scalar fixed array 或元素为扁平 struct 的 fixed array，aggregate value 会在调用 Yul log 前按 ABI 顺序展开。对应 smoke 会检查 golden Yul 可复现、`solc --strict-assembly` 字节码生成、metadata 能力 `events.emit`、Foundry recorded logs（`emitter`、signature topic、1 到 3 个 indexed field 的 scalar indexed topic、indexed aggregate topic hash、来自 local value 和 storage read 的扁平 struct data、来自 local value 和 storage array read 的 scalar fixed-array data、来自 local literal 和 storage array struct field read 的 fixed-array-of-struct data，以及 decoded scalar data）、ABI selector dispatch，以及未知 selector revert。嵌套或不支持形态的 aggregate indexed field 和更完整的 event declaration 仍是 portable IR 的显式 unsupported surface。
