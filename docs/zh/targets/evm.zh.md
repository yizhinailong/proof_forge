...
`EvmArrayValueProbe` 验证 portable IR local fixed-array value。不可变和可变 local fixed-array binding 都会展开为每个元素一个内部 Yul local；对 local array 或 array literal 的 `arrayGet` 支持静态 `U32`/`U64` literal index 和动态 word index。动态读取会降为按长度生成的 Yul helper，动态可变 local 元素赋值和数字复合赋值会降为覆盖展开 local 的 `switch`。whole local fixed-array assignment 从另一个 local fixed-array 或 fixed-array literal 赋值时，会先把 RHS word 快照到临时 local，再写回目标元素。静态嵌套 scalar local fixed-array 会继续展开到 leaf local，例如 `matrix[1][0]` 会变成确定性的 Yul local；whole nested assignment 同样会先快照 RHS leaf 再写回。对应 smoke 覆盖 `U64`、`U32`、`Bool` 和 `Hash` 元素 array、静态/动态可变元素写入、whole-local assignment、静态嵌套 scalar local fixed-array read、静态嵌套 scalar leaf 赋值/复合赋值、带 RHS 快照的嵌套 whole-local assignment、golden Yul 可复现、`solc --strict-assembly`、artifact metadata、Foundry 运行时调用、动态越界 revert，以及未知 selector revert。动态 index 或非 scalar leaf 的嵌套 array 仍保持显式诊断。
