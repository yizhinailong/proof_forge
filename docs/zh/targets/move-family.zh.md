...
Sui 和 Aptos 不是 Wasm 或 EVM 风格的运行时目标。ProofForge 不应尝试将完整的 Lean 运行时编译为 Move。实际的首选路径是从受限的可移植合约 IR 进行源代码生成。
...
Sui 使用以对象为中心的模型。持久状态映射到带有 `UID` 的对象。
...
主要设计风险：Sui 对象所有权并非存储实现细节。它会改变方法签名和调用流，因此必须在可移植 IR 或目标清单中表示。
...
Aptos 可能更容易作为第一个生成的包，因为账户资源更接近传统的存储单元。Sui 在测试抽象方面具有更重要的战略意义，因为它的对象模型与 EVM 相距更远。
...
2. Sui 计数器对象 POC。
3. 比较 IR 差异。
4. 将更简洁的路径提升为第一个 Experimental Move 目标。

## 开放性问题

- Move 包生成应该在 Lean、Zig 还是一个独立的小型生成器中实现？
- 生成的 Move 应该暴露 public view 函数、入口函数，还是两者兼有？
- 通用资产应如何映射到 Sui 和 Aptos 上的 `Coin<T>`？
- Move 的 ability 系统有多少应该在可移植 IR 中建模？
