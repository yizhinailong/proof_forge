...
`EmitWat` 对齐 `ProofForge/Backend/Evm/IR.lean`（portable IR → Yul AST），但目标是 WAT 而不是 Yul。因为 portable IR 已经抽象掉 Lean 对象（只有 `u32`/`u64`/`bool`/`hash` 标量 + storage effects），这里**没有 Lean runtime 要移植，也没有 object-model boxing/GC**。标量直接映射到 Wasm `i32`/`i64`，storage/crypto/context effects lower 到 NEAR host imports。IR-lowering 和 validation logic 可复用冻结 Rust v0（`Backend/WasmHost/IR.lean`）以及 `Backend/Evm/IR.lean`；只有 emission target 变化（Rust/Yul strings → Wasm AST → WAT）。
...
本文档剩余部分描述冻结的 Rust `near-sdk-rs` sourcegen backend（`ProofForge/Backend/WasmHost/IR.lean`）。它作为可工作的 reference 保留，用于验证 NEAR 语义和 capability coverage；它不是 canonical path，也不再扩展。
...
| `ProofForge/Backend/WasmHost/IR.lean` | Core lowering：validation、type inference、Rust source generation（约 57KB） |
