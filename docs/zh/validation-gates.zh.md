...
| EVM ConditionalProbe IR 冒烟测试 | `scripts/evm/conditional-ir-smoke.sh` | `forge`、`solc`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 语句级 `if/else` 降级为 Yul `switch` block，匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，并通过 Foundry 校验 then/else storage 更新结果和未知 selector revert | 分支内早退、更多控制流分析、制品 metadata |
...
| CI 基线 | `.github/workflows/ci.yml` `build-test` 任务 | GitHub Actions Ubuntu、elan、Foundry stable、`solc` 0.8.30 | 清洁环境下的 `lake build`、Psy golden source 快照、Psy 诊断、Psy IR 覆盖清单、EVM 诊断、EVM IR 覆盖清单、EVM ABI ScalarProbe/AssertProbe/AssignmentProbe/ConditionalProbe IR 冒烟测试、EVM 编译和 Foundry 冒烟测试 | 可选 Dargo 目标冒烟测试、metadata 验证、非 Ubuntu 行为 |
