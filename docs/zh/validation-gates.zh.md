...
| EVM 诊断冒烟测试 | `scripts/evm/diagnostic-smoke.sh` | 来自 `lean-toolchain` 的 Lean 工具链 | 不支持或格式错误的 EVM IR 形态在 Yul 生成前以显式诊断失败，包括缺少 selector、不支持的 ABI 类型、缺少返回、Hash/U64 类型错配、array/struct/event/native-value surface、无效 bounded-loop 范围、分支/loop 内 return、crosscall target/method/argument 类型错误、不支持的 map 形态和 `contains`、格式错误的复合赋值类型/target、不支持的 storage path 复合赋值，以及 effect 表达式/语句位置误用（包括 statement 位置的 context read） | 完整 unsupported surface 覆盖、solc 行为、Foundry 运行时行为、制品 metadata |
