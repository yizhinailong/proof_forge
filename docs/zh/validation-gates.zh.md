...
| EVM 诊断冒烟测试 | `scripts/evm/diagnostic-smoke.sh` | 来自 `lean-toolchain` 的 Lean 工具链 | 不支持或格式错误的 EVM IR 形态在 Yul 生成前以显式诊断失败，包括缺少 selector、不支持的 ABI 类型、缺少返回、Hash/U64 类型错配、array/struct/bounded-loop/event/crosscall/native-value surface、不支持的 map 形态和 `contains`、分支内 return，以及 effect 表达式/语句位置误用（包括 statement 位置的 context read） | 完整 unsupported surface 覆盖、solc 行为、Foundry 运行时行为、制品 metadata |
...
| CI 基线 | `.github/workflows/ci.yml` `build-test` 任务 | GitHub Actions Ubuntu、elan、Foundry stable、`solc` 0.8.30 | 清洁环境下的 `lake build`、Psy golden source 快照、Psy 诊断、Psy IR 覆盖清单、EVM 诊断、EVM IR 覆盖清单、EVM ABI ScalarProbe/AssertProbe/AssignmentProbe/ConditionalProbe/ContextProbe/EventProbe/HashProbe/MapProbe IR 冒烟测试、EVM metadata 校验、EVM 编译和 Foundry 冒烟测试 | 可选 Dargo 目标冒烟测试、非 Ubuntu 行为 |
