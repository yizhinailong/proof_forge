...
|---|---|---|---|---|
| 统一 testkit Counter 场景 | `just testkit` | Rust/Cargo；来自 `lean-toolchain` 的 Lean 工具链；EVM bytecode 分支需要 `solc`；Solana/Mollusk 分支可选依赖 `sbpf` 和 `solana-keygen` | RFC 0007 testkit 会发现 `testkit/scenarios/counter.toml`，通过 Lean 发射 Counter WAT fixture 并在确定性的 `runtime/offline-host` wasmtime NEAR host 上运行，同时发射 portable IR Counter 的 EVM runtime bytecode 与 artifact metadata，并在进程内 `revm` 上运行同一组 `initialize`、`get`、`increment`、`get` 场景；如果 Solana 工具链可用，还会发射 Counter sBPF assembly，检查已跟踪的 Solana golden assembly/manifest/artifact metadata，构建 sBPF ELF，并通过 `mollusk-svm` 运行同一个有状态场景；最后校验每个 target 的返回期望，并对已执行 target（工具可用时包括 `wasm-near`、`evm`、`solana-sbpf-asm`）断言规范化 observable trace parity | live NEAR sandbox 部署、live EVM RPC/deployment 行为、Surfpool/Web3 Solana 部署行为、替换旧的逐目标 smoke 脚本 |
