...
| EVM StorageArrayProbe IR 冒烟测试 | `scripts/evm/storage-array-ir-smoke.sh` | `forge`、`solc`、`python3`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR `U64` 固定 storage array 降为连续 EVM storage slot，并带运行时 index bounds check；单段 `index` storage path 会降为同一个 slot helper，用于 read/write/compound assignment；匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，校验包含 `storage.scalar`、`storage.array`、`data.fixed_array` 的 EVM metadata，并通过 Foundry 校验 lifecycle/read/write/path 返回值、`vm.load` 原始 slot layout、越界 index revert 和未知 selector revert | 非 `U64` storage array、local fixed-array value、聚合 ABI 行为由 `EvmAbiAggregateProbe` 覆盖、嵌套 index storage path、struct |
| EVM StorageStructProbe IR 冒烟测试 | `scripts/evm/storage-struct-ir-smoke.sh` | `forge`、`solc`、`python3`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 扁平 scalar storage struct 降为每字段一个 EVM slot，扁平 struct 固定 storage array 降为连续字段展开 slot 并带运行时 index bounds check；scalar `field` 和 array `index`+`field` storage path 会降为同一 slot 公式用于 read/write/compound assignment；生成的 Yul 匹配 golden fixture，通过 `solc --strict-assembly` 编译，metadata 包含 `storage.scalar`、`storage.array`、`data.fixed_array` 和 `data.struct`，Foundry 校验 U64/Bool/U32/Hash 字段、原始 slot layout、越界 revert 和未知 selector | Whole-struct storage read/write、嵌套 struct field、非扁平 struct storage、非 `U64` storage array |
| EVM ArrayValueProbe IR 冒烟测试 | `scripts/evm/array-value-ir-smoke.sh` | `forge`、`solc`、`python3`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 不可变 local fixed-array value 会降为每个元素一个 Yul local；fixed-array literal 和静态 index read 可确定性 lowering；匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，校验包含 `data.fixed_array` 和 `assertions.check` 的 EVM metadata，并通过 Foundry 校验 U64/U32/Bool/Hash 返回值和未知 selector revert | 动态 local fixed-array index、mutable local fixed array、ABI 聚合行为由 `EvmAbiAggregateProbe` 覆盖、嵌套 array、U64 之外的 storage array |
| EVM StructValueProbe IR 冒烟测试 | `scripts/evm/struct-value-ir-smoke.sh` | `forge`、`solc`、`python3`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 扁平不可变 local struct value 会降为每个字段一个 Yul local；struct literal 和 field access 针对 U64/U32/Bool/Hash 字段可确定性 lowering；匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，校验包含 `data.struct` 和 `assertions.check` 的 EVM metadata，并通过 Foundry 校验 U64/U32/Bool/Hash 返回值和未知 selector revert | mutable local struct、嵌套 struct、whole-struct storage read/write、ABI 聚合行为由 `EvmAbiAggregateProbe` 覆盖、struct array |
| EVM AbiAggregateProbe IR 冒烟测试 | `scripts/evm/abi-aggregate-ir-smoke.sh` | `forge`、`solc`、`python3`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 扁平静态 struct 和 fixed-array ABI 参数会展开为连续 calldata word，`U32` 和 `Bool` 聚合 word 会带 dispatcher range guard，扁平 struct 和 fixed-array 返回会编码为多 word ABI return data；生成的 Yul 匹配 golden fixture，通过 `solc --strict-assembly` 编译，metadata 包含 `data.fixed_array` 和 `data.struct`，Foundry 校验聚合调用/返回、malformed calldata revert 和未知 selector revert | 嵌套/动态聚合 ABI 值、storage-backed aggregate ABI 值、超出扁平 word 聚合的复杂 ABI tuple |
| EVM 诊断冒烟测试 | `scripts/evm/diagnostic-smoke.sh` | 来自 `lean-toolchain` 的 Lean 工具链 | 不支持或格式错误的 EVM IR 形态在 Yul 生成前以显式诊断失败，包括缺少 selector、Unit/零长度/嵌套聚合 ABI 参数等不支持 ABI 值、缺少返回、Hash/U64 类型错配、array/struct/event surface、无效 bounded-loop 范围、分支/loop 内 return、crosscall target/method/argument 类型错误、不支持的 map 形态和 `contains`、格式错误的复合赋值类型/target、storage path 复合赋值的表达式位置/嵌套路径误用，以及 effect 表达式/语句位置误用（包括 statement 位置的 context read） | 完整 unsupported surface 覆盖、solc 行为、Foundry 运行时行为、制品 metadata |
