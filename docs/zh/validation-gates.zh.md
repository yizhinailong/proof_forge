...
|---|---|---|---|---|
| 统一 testkit Counter 场景 | `just testkit` | Rust/Cargo；来自 `lean-toolchain` 的 Lean 工具链；EVM bytecode 分支需要 `solc` | RFC 0007 testkit 会发现 `testkit/scenarios/counter.toml`，通过 Lean 发射 Counter WAT fixture 并在确定性的 `runtime/offline-host` wasmtime NEAR host 上运行，同时发射 portable IR Counter 的 EVM runtime bytecode 与 artifact metadata，并在进程内 `revm` 上运行同一组 `initialize`、`get`、`increment`、`get` 场景，最后校验两个 target 的返回期望，并断言 `wasm-near` 与 `evm` 的规范化 observable trace parity | Solana/Mollusk 执行、live NEAR sandbox 部署、live EVM RPC/deployment 行为、替换旧的逐目标 smoke 脚本 |
