...
| EVM ABI ScalarProbe IR 冒烟测试 | `scripts/evm/abi-scalar-ir-smoke.sh` | `forge`、`solc`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR `U64`、`U32` 和 `Bool` ABI 参数降级为 Yul 函数参数和 dispatcher calldata load，匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，并通过 Foundry 校验有效调用和 malformed calldata revert | 聚合 ABI 参数/返回、storage 行为、制品 metadata |
| EVM AssertProbe IR 冒烟测试 | `scripts/evm/assert-ir-smoke.sh` | `forge`、`solc`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR `assert` 和 `assert_eq` 降级为 Yul revert guard，匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，并通过 Foundry 校验成功路径和断言失败 revert | 丰富 revert data、表达式类型检查、制品 metadata |
| EVM AssignmentProbe IR 冒烟测试 | `scripts/evm/assignment-ir-smoke.sh` | `forge`、`solc`、来自 `lean-toolchain` 的 Lean 工具链 | Portable IR 可变标量 local 和 local assignment 降级为 Yul `let` 与 `:=`，匹配 golden Yul，通过 `solc --strict-assembly` 生成 runtime bytecode，并通过 Foundry 校验赋值结果和 bool guard 失败 revert | 复合赋值、聚合赋值路径、storage 赋值路径、制品 metadata |
| EVM 诊断冒烟测试 | `scripts/evm/diagnostic-smoke.sh` | 来自 `lean-toolchain` 的 Lean 工具链 | 不支持或格式错误的 EVM IR 形态在 Yul 生成前以显式诊断失败，包括缺少 selector、不支持的 ABI 类型、缺少返回、Hash/array/struct/control/storage/context/event/crosscall/native-value surface，以及 effect 表达式/语句位置误用 | 完整 unsupported surface 覆盖、solc 行为、Foundry 运行时行为、制品 metadata |
...
| CI 基线 | `.github/workflows/ci.yml` `build-test` 任务 | GitHub Actions Ubuntu、elan、Foundry stable、`solc` 0.8.30 | 清洁环境下的 `lake build`、Psy golden source 快照、Psy 诊断、Psy IR 覆盖清单、EVM 诊断、EVM IR 覆盖清单、EVM ABI ScalarProbe/AssertProbe/AssignmentProbe IR 冒烟测试、EVM 编译和 Foundry 冒烟测试 | 可选 Dargo 目标冒烟测试、metadata 验证、非 Ubuntu 行为 |
