...
state-machine rules, and formal proofs once in Lean~4, then compile to chain-native artifacts
across multiple target families through a single portable intermediate representation (IR).
...
registry with 23 registered capability identifiers across 9 target profiles, and three
production-grade backends---EVM (via Yul/solc), Solana sBPF (direct assembly), and NEAR/Wasm
(via EmitWat). The platform includes executable IR semantics with \texttt{decide}-checked
trace obligations, ownership rules for heap-backed locals, and a staged formal verification
roadmap (FV-1 through FV-8). The unified testkit achieves behavior parity and resource budget
baselines for the Counter and ValueVault shared scenarios across all three primary targets.
Gate~P0---the primary-chain completion covenant---is closed as of July~2026.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{One verified source, multiple targets.} Contract logic, state-machine rules,
  and invariants are written once in Lean~4~\cite{lean4}. The compiler lowers them through a
  ...
  \item \textbf{Proofs gate codegen.} Formal verification is integrated into the pipeline:
  proofs are checked in Lean before backend lowering, not retrofitted after deployment.
\end{enumerate}

ProofForge currently supports three production-grade backends: EVM (via Yul and
...
\begin{itemize}[leftmargin=*]
  ...
  (\S\ref{sec:arch}).
  \item A \textbf{capability routing model} with 23 registered capability identifiers across 9
  target profiles, enabling compile-time rejection of unsupported operations (\S\ref{sec:cap}).
  \item Three \textbf{production-grade backend implementations}---EVM/Yul, Solana/sBPF-asm, and
  NEAR/EmitWat---each with full artifact emission, deployment metadata, and validation gates
  (\S\ref{sec:backends}).
...
Table~\ref{tab:capreg} shows the support matrix for primary and representative targets.
...
An ownership checker (\texttt{ProofForge.IR.Ownership.lean}) validates that released locals are
never used afterward (no use-after-release) and never released twice (no double-release). Each
target configures its allocator via \texttt{AllocatorConfig}; see Table~\ref{tab:alloc}.
...
The EVM SDK (\texttt{ProofForge.Evm}) provides extern declarations for all EVM opcodes
(\texttt{calldataload}, \texttt{sstore}, \texttt{sload}, \texttt{caller}, \texttt{callvalue},
\texttt{keccak256}, \texttt{log0}--\texttt{log4}, etc.) that are recognized by EmitYul and lowered
directly to EVM instructions. Typed storage helpers (\texttt{Storage.Var}, \texttt{Storage.Map},
\texttt{Storage.Map2}) provide compile-time slot assignment and mapping slot computation via
\texttt{keccak256(key || slot)}.
...
The backend defines Solana-specific constructs in \texttt{ProofForge.Solana}:
\texttt{AccountMeta} (writable/read-only accounts with signer policies),
\texttt{PdaBinding} (program-derived address derivation with seeds and bump),
\texttt{CpiCall} (cross-program invocations with account metas and signer seeds),
\texttt{SysvarReadAction} (system variable reads: rent, epoch schedule, etc.),
\texttt{MemoryAction} (runtime memory operations: memcpy, memset, memcmp),
\texttt{CryptoHashAction} (SHA-256, Keccak-256, Blake3), and
\texttt{ComputeUnitsAction} (compute budget introspection).
...
EmitWat avoids the macro coupling of Rust \texttt{near-sdk-rs} source generation (D-031) and
the Lean-runtime-to-Wasm port that blocked prior approaches. Because the portable IR already
abstracts over Lean objects (only \texttt{u32}/\texttt{u64}/\texttt{bool}/\texttt{hash} scalars
+ storage effects), EmitWat needs no Lean runtime port, object-model boxing, or garbage
collection.

The NEAR backend includes target-first \texttt{check}, \texttt{emit}, and \texttt{build}
commands, writes \texttt{proof-forge-artifact.json} and \texttt{proof-forge-deploy.json},
validates WAT/Wasm hashes, ABI entrypoints, and capabilities, and executes the generated
Counter WAT through an offline host.

\subsection{Research and Frozen Backends}

Additional backends exist in research or frozen-spike stages:
\textbf{Psy/DPN} (portable IR $\to$ \texttt{.psy} $\to$ Dargo $\to$ DPN circuit JSON, experimental),
\textbf{Aleo Leo} (portable IR $\to$ Leo package, research spike with golden fixtures~\cite{aleo}),
\textbf{Cloudflare Workers} (portable IR $\to$ TypeScript Worker, off-chain host),
\textbf{Move/Aptos}~\cite{move} (frozen at M1/M2 spike per D-044), and
\textbf{CosmWasm}~\cite{cosmwasm} (frozen at M1/M2 spike).
...
ProofForge is Lean-first, but most current assurance comes from tests and golden files rather
than theorems. The formal verification roadmap (FV-1 through FV-8) progressively turns
test-validated invariants into machine-checked theorems. Proofs gate codegen, per RFC~0001:
no proof transport to chains, no on-chain verification of Lean proofs.

\subsection{Existing Formal Anchors}

Three formal anchors are already implemented on \texttt{main}; see Table~\ref{tab:fv}.
...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    \midrule
    IR Semantics \scriptsize(\texttt{IR/Semantics.lean}) & Trace interpreter for scalars, arrays, structs, storage, control flow, events; \texttt{decide}-checked \\
    \addlinespace
    Ownership Rules \scriptsize(\texttt{IR/Ownership.lean}) & \texttt{release}/owned-local discipline; no use-after-release, no double-release \\
    \addlinespace
    Trace Obligations \scriptsize(\texttt{Backend/*/Refinement.lean}) & \texttt{decide}-checked theorems: Counter, ValueVault + 8 probe traces match expected observable values on EVM Yul and NEAR WAT \\
    \bottomrule
  \end{tabularx}
\end{table*}
...
\textbf{FV-1 (Capability Routing Soundness):} Prove that if
\texttt{resolveSpec profile spec = .ok plan}, then every capability in
\texttt{plan.calls} is in \texttt{profile.capabilities} (soundness), and that if the spec
references a capability outside the profile, \texttt{resolveSpec} returns \texttt{.error}
(completeness of rejection). These are structural inductions over arrays---no semantics needed.
...
release twice. This is the formal justification for lowering \texttt{release} to allocator frees
in EmitWat while EVM/Psy reject it and TS ignores it.
...
\begin{table*}[t]
  \caption{Validation gate coverage per primary target.}
...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{l X X X}
    ...
    Formal proofs          & Integrated (Lean) & Linear temporal logic & No \\
    Capability model       & 23 IDs, 9 profiles & Network-specific & EVM-centric \\
...
priority chains must be completed to production-grade quality before any additional chain
...
ProofForge demonstrates that a Lean-first, capability-routed, portable-IR-centered architecture
can produce production-grade smart contract artifacts across heterogeneous blockchain platforms
without silently changing semantics. The platform's core promise---\emph{one verified contract
source, multiple chain-native artifacts, unsupported operations rejected at compile time}---is
validated by the closure of Gate~P0, with behavior parity and resource budgets confirmed across
EVM, Solana sBPF, and NEAR/Wasm.
