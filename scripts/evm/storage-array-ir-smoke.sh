...
GOLDEN_FILE="${IR_EVM_GOLDEN:-$ROOT/Examples/Backend/Evm/EvmStorageArrayProbe.golden.yul}"
...
contract ProofForgeIRStorageArraySmokeTest {
    ...
    function testIRStorageArrayLifecycleUsesContiguousSlots() public {
        ...
        assertEq(readStorage(probe, bytes32(uint256(0))) & type(uint64).max, 111);
        ...
        assertEq(readStorage(probe, bytes32(uint256(4))) & type(uint64).max, 222);
    }
