#!/usr/bin/env python3
"""Validate that the wasm-near IR coverage manifest tracks all portable IR variants."""
...
def read_ir_constructors(path: Path) -> set[tuple[str, str]]:
    ...
    for line_no, line in enumerate(path.read_text().splitlines(), start=1):
        ...
        if constructor_match:
            ...
            raise SystemExit(
                f"wasm-near-ir-coverage: could not parse constructor at {path}:{line_no}: {line}"
            )
