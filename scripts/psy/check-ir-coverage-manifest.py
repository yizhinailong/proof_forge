#!/usr/bin/env python3
"""Validate that the Psy IR coverage manifest tracks all portable IR variants."""
...
def read_ir_constructors(path: Path) -> set[tuple[str, str]]:
    ...
    for line_no, line in enumerate(path.read_text().splitlines(), start=1):
        ...
        if constructor_match:
            ...
            raise SystemExit(
                f"psy-ir-coverage: could not parse constructor at {path}:{line_no}: {line}"
            )
...
def main() -> int:
    ...
    parser.add_argument("--manifest", default="Tests/PsyCoverage.tsv")
    ...
    if errors:
        for error in errors:
            print(f"psy-ir-coverage: {error}", file=sys.stderr)
        return 1

    print(f"psy-ir-coverage: {len(manifest)} constructor entries match {ir_path}")
    return 0
