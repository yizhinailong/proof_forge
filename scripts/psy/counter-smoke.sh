...
# Generate Psy source from the hand-written portable Counter IR and validate it
# through Psy's official Dargo toolchain.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT_DIR="${PSY_OUT_DIR:-$ROOT/build/psy}"
PROJECT_DIR="$OUT_DIR/dargo-counter"
PSY_FILE="$OUT_DIR/Counter.psy"
GOLDEN_FILE="${PSY_GOLDEN:-$ROOT/Examples/Backend/Psy/Counter.golden.psy}"
...
if ! command -v "$DARGO_BIN" >/dev/null 2>&1; then
  echo "psy-counter-smoke: dargo not found. Install the Psy toolchain with psyup, then re-run this script." >&2
  echo "psy-counter-smoke: generated $PSY_FILE for inspection." >&2
  echo "psy-counter-smoke: install: curl -fsSL https://raw.githubusercontent.com/QEDProtocol/psyup/main/install.sh | bash" >&2
  echo "psy-counter-smoke: macOS arm64 note: psyup latest may not have a matching tarball; v0.1.0 is known to include one." >&2
  echo "psy-counter-smoke: docs: https://docs.psy-protocol.xyz/language/dargo.html" >&2
  exit 127
fi
...
python3 "$ROOT/scripts/psy/write-dargo-package.py" \
  ...
  --source "$PSY_FILE" \
  --package-name "proof_forge_counter" \
  --description "ProofForge generated Counter IR Psy smoke"
...
if [[ ! -s "$ARTIFACT" ]]; then
  echo "psy-counter-smoke: expected non-empty Dargo artifact at $ARTIFACT" >&2
  exit 1
fi

if ! grep -Fq "result_vm: [2]" "$EXEC_LOG"; then
  echo "psy-counter-smoke: expected dargo execute to return result_vm: [2]" >&2
  echo "psy-counter-smoke: execution log: $EXEC_LOG" >&2
  exit 1
fi

if [[ ! -s "$ABI_FILE" ]]; then
  echo "psy-counter-smoke: expected non-empty Dargo ABI at $ABI_FILE" >&2
  exit 1
fi
