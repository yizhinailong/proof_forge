...
# harness, invokes the generated program, and verifies that the canonical
# associated token account was created while state records the call.
#
# Exit codes:
#   0 - all gates passed
#   1 - a gate failed
...
echo "=== Solana Associated Token CPI step 5: run Rust behavior checks ==="
...
PROOF_FORGE_SOLANA_ARTIFACT="$ARTIFACT_OUTPUT" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin associated_token_cpi_live_smoke \
  || fail "Rust Associated Token CPI checks failed"

echo "=== Solana Associated Token CPI Surfpool/Rust smoke: PASS ==="
