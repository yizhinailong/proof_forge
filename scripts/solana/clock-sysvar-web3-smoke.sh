...
# CLI, invokes the program through @solana/web3.js, and verifies that
...
JS_TEMPLATE="$REPO_ROOT/Tests/solana/clock_sysvar_web3_smoke.mjs"
...
echo "=== Solana Clock sysvar step 5: run Web3.js behavior checks ==="
cp "$JS_TEMPLATE" "$NODE_PROJECT/clock_sysvar_web3_smoke.mjs"
...
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  node "$NODE_PROJECT/clock_sysvar_web3_smoke.mjs" \
  || fail "Web3.js Clock sysvar checks failed"

echo "=== Solana Clock sysvar Surfpool/Web3.js smoke: PASS ==="
