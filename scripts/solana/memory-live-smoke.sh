...
# account bytes after sol_memcpy_, sol_memmove_, sol_memcmp_, and sol_memset_
# execute on chain.
#
# Exit codes:
#   0 - all gates passed
...
echo "=== Solana memory step 5: run Rust behavior checks ==="
PROOF_FORGE_SOLANA_RPC_URL="$RPC_URL" \
PROOF_FORGE_SOLANA_PAYER="$PAYER_KEYPAIR" \
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin memory_syscall_live_smoke \
  || fail "Rust memory checks failed"

echo "=== Solana memory Surfpool/Rust smoke: PASS ==="
