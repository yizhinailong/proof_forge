#!/usr/bin/env bash
# ProofForge Solana SPL Token authority Pinocchio reference-equivalence smoke.
#
# Emits the ProofForge SPL Token set_authority CPI source fixture and compares
...
OUT_DIR="${PROOF_FORGE_PINOCCHIO_SPL_TOKEN_AUTHORITY_OUT:-build/solana-pinocchio-spl-token-authority-equivalence}"
...
ASM_OUTPUT="$OUT_DIR/proofforge-spl-token-authority-reference.s"
...
echo "=== Pinocchio SPL Token authority equivalence step 1: emit ProofForge source fixture ==="
lake env proof-forge emit --target solana-sbpf-asm --fixture spl-token-authority-cpi --format s \
  -o "$ASM_OUTPUT" \
  --artifact-output "$ARTIFACT_OUTPUT" \
  || fail "proof-forge emit --target solana-sbpf-asm --fixture spl-token-authority-cpi --format s failed"
[ -f "$ARTIFACT_OUTPUT" ] || fail "artifact not produced: $ARTIFACT_OUTPUT"

echo "=== Pinocchio SPL Token authority equivalence step 2: compare contracts ==="
...
for cpi, ref_cpi in zip(cpis, ref_cpis):
    for key in ("name", "program", "protocol", "instruction", "dataLayout",
                "authorityType", "newAuthority", "signed"):
...
expected_constants = {
    "PF_ENTRYPOINT_TAG": reference_entrypoints[0]["tag"],
    "PF_MIN_INSTRUCTION_DATA_LEN": reference_entrypoints[0]["minDataLen"],
    "PF_ACCOUNT_COUNT": len(reference_accounts),
    "PF_STATE_ACCOUNT_INDEX": 0,
    "PF_MINT_ACCOUNT_INDEX": 1,
    "PF_AUTHORITY_ACCOUNT_INDEX": 2,
    "PF_SPL_TOKEN_ACCOUNT_INDEX": 3,
    "PF_NEW_AUTHORITY_ACCOUNT_INDEX": 4,
    "PF_TOKEN_SET_AUTHORITY_DISCRIMINATOR": instruction_data["discriminator"],
    "PF_TOKEN_AUTHORITY_TYPE_MINT_TOKENS": instruction_data["authorityType"],
    "PF_TOKEN_NEW_AUTHORITY_OPTION": instruction_data["newAuthorityOption"],
    "PF_TOKEN_SET_AUTHORITY_DATA_LEN": instruction_data["dataLen"],
    "PF_STATE_WRITE_OFFSET": reference["stateWrites"][0]["offset"],
    "PF_STATE_WRITE_SIZE": reference["stateWrites"][0]["byteSize"],
    "PF_STATE_MARKER": reference["stateWrites"][0]["value"],
}
...
if [ "${PROOF_FORGE_PINOCCHIO_CARGO_CHECK:-0}" = "1" ]; then
  ...
  echo "=== Pinocchio SPL Token authority equivalence step 3: optional cargo check ==="
...
echo "=== Pinocchio SPL Token authority reference-equivalence smoke: PASS ==="
