...
#
# Emits the ProofForge SPL Token set_authority CPI fixture and compares its
...
ELF_OUTPUT="$OUT_DIR/proofforge-spl-token-authority-reference.so"
...
echo "=== Pinocchio SPL Token authority equivalence step 1: emit ProofForge fixture ==="
lake env proof-forge --solana-spl-token-authority-cpi-elf --solana-sbpf-arch "$SBPF_ARCH" \
  -o "$ELF_OUTPUT" \
  --artifact-output "$ARTIFACT_OUTPUT" \
  || fail "proof-forge --solana-spl-token-authority-cpi-elf failed"
