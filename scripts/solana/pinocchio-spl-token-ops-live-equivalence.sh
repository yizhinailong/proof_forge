...
# Surfpool instance, invokes the same Web3.js token operations scenario against
...
echo "=== Pinocchio SPL Token ops live equivalence step 6: run Web3.js behavior checks ==="
...
PROOF_FORGE_SOLANA_TOKEN_APPROVE_AMOUNT="$APPROVE_AMOUNT" \
  node "$NODE_PROJECT/spl_token_ops_cpi_web3_smoke.mjs" > "$OUT_DIR/proofforge-result.json" \
  || fail "Web3.js ProofForge SPL Token ops CPI checks failed"
PROOF_FORGE_SOLANA_RPC_URL="$RPC_URL" \
PROOF_FORGE_SOLANA_WS_URL="$WS_URL" \
PROOF_FORGE_SOLANA_PAYER="$PAYER_KEYPAIR" \
...
PROOF_FORGE_SOLANA_TOKEN_BURN_AMOUNT="$BURN_AMOUNT" \
PROOF_FORGE_SOLANA_TOKEN_APPROVE_AMOUNT="$APPROVE_AMOUNT" \
  node "$NODE_PROJECT/spl_token_ops_cpi_web3_smoke.mjs" > "$OUT_DIR/pinocchio-result.json" \
  || fail "Web3.js Pinocchio SPL Token ops CPI checks failed"
