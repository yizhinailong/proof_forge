#!/usr/bin/env bash
# ProofForge vs Pinocchio System create_account live equivalence on Surfpool.
#
...
OUT_DIR="${PROOF_FORGE_PINOCCHIO_SYSTEM_CREATE_ACCOUNT_LIVE_OUT:-build/solana-pinocchio-system-create-account-live}"
REFERENCE_DIR="$REPO_ROOT/references/solana/pinocchio/system-create-account"
PROOF_FORGE_PROJECT_NAME="proofforge-system-create-account-live"
PINOCCHIO_PROJECT_NAME="pinocchio-system-create-account-reference"
...
RPC_PORT="${PROOF_FORGE_PINOCCHIO_SYSTEM_CREATE_ACCOUNT_SURFPOOL_PORT:-8910}"
WS_PORT="${PROOF_FORGE_PINOCCHIO_SYSTEM_CREATE_ACCOUNT_SURFPOOL_WS_PORT:-8911}"
RPC_URL="http://$RPC_HOST:$RPC_PORT"
SURFPOOL_LOG_DIR="$OUT_DIR/surfpool-logs"
SPACE="${PROOF_FORGE_SOLANA_CREATE_SPACE:-24}"
SURFPOOL_PID=""
...
Then rerun:
  PROOF_FORGE_PINOCCHIO_USE_RUSTUP=1 scripts/solana/pinocchio-system-create-account-live-equivalence.sh
EOF
}
...
echo "=== Pinocchio create_account live equivalence step 3: generate local keypairs ==="
...
echo "=== Pinocchio create_account live equivalence step 4: start Surfpool ==="
"$SURFPOOL_BIN" start \
  --host "$RPC_HOST" \
  --port "$RPC_PORT" \
  --ws-port "$WS_PORT" \
...
echo "=== Pinocchio create_account live equivalence step 5: deploy both programs ==="
...
for label, result in (("ProofForge", proof), ("Pinocchio", reference)):
    require(result["recordedLamports"] == result["lamports"],
            f"{label} lamports state write mismatch: {result}")
    require(result["recordedSpace"] == result["space"],
            f"{label} space state write mismatch: {result}")

for field in ("lamports", "space", "recordedLamports", "recordedSpace"):
...
print(json.dumps({
    ...
    "space": proof["space"],
    "proofForgeRecordedLamports": proof["recordedLamports"],
    "proofForgeRecordedSpace": proof["recordedSpace"],
    "pinocchioRecordedLamports": reference["recordedLamports"],
    "pinocchioRecordedSpace": reference["recordedSpace"],
}, sort_keys=True))
PY

echo "=== Pinocchio System create_account live equivalence: PASS ==="
