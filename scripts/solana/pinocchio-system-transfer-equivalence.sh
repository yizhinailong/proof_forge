...
OUT_DIR="${PROOF_FORGE_PINOCCHIO_SYSTEM_TRANSFER_OUT:-build/solana-pinocchio-system-transfer-equivalence}"
...
require(cpi.get("lamportsSource") == ref_cpi["lamportsSource"],
        f"CPI lamports source mismatch: {cpi}")
require(cpi.get("signed") == ref_cpi["signed"], f"CPI signed mismatch: {cpi}")
...
expected_constants = {
    ...
    "PF_LAMPORTS_OFFSET": ref_entry["params"][0]["offset"],
    "PF_LAMPORTS_SIZE": ref_entry["params"][0]["byteSize"],
    "PF_ACCOUNT_COUNT": len(reference_accounts),
    "PF_STATE_ACCOUNT_INDEX": 0,
    "PF_PAYER_ACCOUNT_INDEX": 1,
    "PF_RECIPIENT_ACCOUNT_INDEX": 2,
...
if [ "${PROOF_FORGE_PINOCCHIO_CARGO_CHECK:-0}" = "1" ]; then
  ...
  echo "=== Pinocchio System transfer equivalence step 3: optional cargo check ==="
...
echo "=== Pinocchio System transfer reference-equivalence smoke: PASS ==="
