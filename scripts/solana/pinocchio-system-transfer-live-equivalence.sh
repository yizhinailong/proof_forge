...
# invokes the same Rust live System transfer scenario against each program, and
...
PINOCCHIO_LIVE_SCRIPT="scripts/solana/pinocchio-system-transfer-live-equivalence.sh"
...
echo "=== Pinocchio live equivalence step 6: run Rust behavior checks ==="
...
PROOF_FORGE_SOLANA_TRANSFER_LAMPORTS="$LAMPORTS" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin system_cpi_live_smoke > "$OUT_DIR/proofforge-result.json" \
  || fail "Rust ProofForge System CPI checks failed"
PROOF_FORGE_SOLANA_RPC_URL="$RPC_URL" \
PROOF_FORGE_SOLANA_PAYER="$PAYER_KEYPAIR" \
PROOF_FORGE_SOLANA_PROGRAM_ID="$PINOCCHIO_PROGRAM_ID" \
PROOF_FORGE_SOLANA_TRANSFER_LAMPORTS="$LAMPORTS" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin system_cpi_live_smoke > "$OUT_DIR/pinocchio-result.json" \
  || fail "Rust Pinocchio System CPI checks failed"
