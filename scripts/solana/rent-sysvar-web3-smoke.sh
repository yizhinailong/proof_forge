...
# the SDK sysvar target extension records Rent.lamports_per_byte_year into a
...
echo "=== Solana Rent sysvar step 5: run Rust behavior checks ==="
PROOF_FORGE_SOLANA_RPC_URL="$RPC_URL" \
PROOF_FORGE_SOLANA_PAYER="$PAYER_KEYPAIR" \
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin rent_sysvar_live_smoke \
  || fail "Rust Rent sysvar checks failed"

echo "=== Solana Rent sysvar Surfpool/Rust smoke: PASS ==="
