#!/usr/bin/env bash
# ProofForge Solana Rent sysvar live smoke on Surfpool.
#
# Builds the generated Rent sysvar ELF, starts Surfpool, deploys with Solana
# CLI, invokes the program through @solana/web3.js, and verifies that the
# SDK sysvar target extension records Rent.lamports_per_byte_year into a
...
OUT_DIR="${PROOF_FORGE_SOLANA_RENT_OUT:-build/solana-rent-sysvar-live}"
PROJECT_NAME="proofforge-rent-sysvar-live"
...
JS_TEMPLATE="$REPO_ROOT/Tests/solana/rent_sysvar_web3_smoke.mjs"
NODE_PROJECT="$OUT_DIR/web3"
SURFPOOL_BIN="${SURFPOOL:-surfpool}"
SOLANA_BIN="${SOLANA:-solana}"
KEYGEN="${SOLANA_KEYGEN:-solana-keygen}"
NPM_BIN="${NPM:-npm}"
SBPF_ARCH="${PROOF_FORGE_SOLANA_RENT_SBPF_ARCH:-v0}"
RPC_HOST="${PROOF_FORGE_SURFPOOL_HOST:-127.0.0.1}"
RPC_PORT="${PROOF_FORGE_RENT_SURFPOOL_PORT:-8905}"
WS_PORT="${PROOF_FORGE_RENT_SURFPOOL_WS_PORT:-8906}"
...
echo "=== Solana Rent sysvar step 1: build fixture ELF ==="
lake env proof-forge emit --target solana-sbpf-asm --fixture solana-rent-sysvar --format elf --solana-sbpf-arch "$SBPF_ARCH" \
  -o "$ELF_OUTPUT" \
  --artifact-output "$ARTIFACT_OUTPUT" \
  || fail "proof-forge emit --target solana-sbpf-asm --fixture solana-rent-sysvar --format elf failed"
...
if artifact.get("fixture") != "solana-rent-sysvar-elf":
...
if len(instructions) != 1 or instructions[0].get("name") != "record_rent":
...
if accounts != ["lamports_per_byte_year"]:
...
echo "=== Solana Rent sysvar step 2: generate local keypairs ==="
...
echo "=== Solana Rent sysvar step 3: start Surfpool ==="
"$SURFPOOL_BIN" start \
  --host "$RPC_HOST" \
  --port "$RPC_PORT" \
  --ws-port "$WS_PORT" \
...
echo "=== Solana Rent sysvar step 4: deploy program ==="
...
echo "=== Solana Rent sysvar step 5: run Web3.js behavior checks ==="
cp "$JS_TEMPLATE" "$NODE_PROJECT/rent_sysvar_web3_smoke.mjs"
...
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  node "$NODE_PROJECT/rent_sysvar_web3_smoke.mjs" \
  || fail "Web3.js Rent sysvar checks failed"

echo "=== Solana Rent sysvar Surfpool/Web3.js smoke: PASS ==="
