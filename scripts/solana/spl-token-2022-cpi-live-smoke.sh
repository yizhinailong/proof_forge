#!/usr/bin/env bash
# ProofForge Solana Token-2022 direct CPI live smoke on Surfpool.
set -euo pipefail
...
IDL_OUTPUT="$OUT_DIR/$PROJECT_NAME-sbpf-project/proof-forge-idl.json"
...
[ -f "$IDL_OUTPUT" ] || fail "IDL not produced: $IDL_OUTPUT"
...
if list(idl_cpis) != list(expected_cpis):
    raise SystemExit(f"IDL CPI schema mismatch: {list(idl_cpis)}")
...
if idl_cpis["token_2022_init_metadata_pointer"].get("metadataPointerAuthority") != "metadata_pointer_authority":
    raise SystemExit("metadata_pointer authority source mismatch")
if idl_cpis["token_2022_init_metadata_pointer"].get("metadataAddress") != "metadata_address":
    raise SystemExit("metadata_pointer address source mismatch")
if idl_cpis["token_2022_init_default_account_state"].get("defaultAccountState") != "2":
    raise SystemExit("default_account_state mismatch")
if idl_cpis["token_2022_init_permanent_delegate"].get("permanentDelegate") != "permanent_delegate":
    raise SystemExit("permanent_delegate source mismatch")
if idl_cpis["token_2022_init_interest_bearing"].get("interestRateAuthority") != "interest_rate_authority":
    raise SystemExit("interest_rate_authority source mismatch")
if idl_cpis["token_2022_init_interest_bearing"].get("interestRate") != "250":
    raise SystemExit("interest_rate mismatch")
if idl_cpis["token_2022_enable_memo_transfer"].get("memoTransferRequired") != "true":
    raise SystemExit("memo_transfer required flag mismatch")
if idl_cpis["token_2022_init_transfer_hook"].get("transferHookAuthority") != "transfer_hook_authority":
    raise SystemExit("transfer_hook authority source mismatch")
if idl_cpis["token_2022_init_transfer_hook"].get("transferHookProgram") != "transfer_hook_program":
...
echo "=== Solana Token-2022 direct CPI step 5: run Rust behavior checks ==="
...
PROOF_FORGE_SOLANA_ARTIFACT="$ARTIFACT_OUTPUT" \
  cargo run --quiet --manifest-path testkit/Cargo.toml \
    ...
    --bin spl_token_2022_cpi_live_smoke \
  || fail "Rust Token-2022 direct CPI checks failed"

echo "=== Solana Token-2022 direct CPI Surfpool/Rust smoke: PASS ==="
