...
echo "=== Solana Token-2022 transfer-hook step 5: run Rust behavior checks ==="
...
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  cargo run --quiet --manifest-path testkit/Cargo.toml \
    ...
    --bin spl_token_2022_transfer_hook_live_smoke \
  || fail "Rust Token-2022 transfer-hook checks failed"

echo "=== Solana Token-2022 transfer-hook Surfpool/Rust smoke: PASS ==="
