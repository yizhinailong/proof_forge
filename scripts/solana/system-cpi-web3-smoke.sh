...
OUT_DIR="${PROOF_FORGE_SOLANA_SYSTEM_CPI_OUT:-build/solana-system-cpi-live}"
PROJECT_NAME="proofforge-system-cpi-live"
...
WS_PORT="${PROOF_FORGE_SYSTEM_CPI_SURFPOOL_WS_PORT:-8897}"
...
if artifact.get("fixture") != "solana-system-cpi-elf":
...
echo "=== Solana System CPI step 3: start Surfpool ==="
"$SURFPOOL_BIN" start \
  --host "$RPC_HOST" \
  --port "$RPC_PORT" \
  --ws-port "$WS_PORT" \
...
echo "=== Solana System CPI step 4: deploy program ==="
