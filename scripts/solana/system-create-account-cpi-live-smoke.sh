#!/usr/bin/env bash
# ProofForge Solana System Program create_account CPI live smoke on Surfpool.
#
# Builds the System create_account CPI SDK fixture, starts Surfpool, deploys the
# generated ELF with Solana CLI, and invokes it through the Rust live RPC
# harness. The program creates a program-owned account through CPI and records
# the requested lamports and space in its state account.
#
# Exit codes:
#   0 - all gates passed
#   1 - a gate failed
...
OUT_DIR="${PROOF_FORGE_SOLANA_SYSTEM_CREATE_ACCOUNT_CPI_OUT:-build/solana-system-create-account-cpi-live}"
PROJECT_NAME="proofforge-system-create-account-cpi-live"
...
RPC_PORT="${PROOF_FORGE_SYSTEM_CREATE_ACCOUNT_CPI_SURFPOOL_PORT:-8898}"
WS_PORT="${PROOF_FORGE_SYSTEM_CREATE_ACCOUNT_CPI_SURFPOOL_WS_PORT:-8897}"
...
if artifact.get("fixture") != "solana-system-create-account-cpi-elf":
...
expected_params = [
    {"name": "lamports", "type": "U64", "offset": 1, "byteSize": 8, "encoding": "le-u64"},
    {"name": "space", "type": "U64", "offset": 9, "byteSize": 8, "encoding": "le-u64"},
]
if params != expected_params:
    raise SystemExit(f"parameter schema mismatch: {params}")
cpis = artifact.get("solanaExtensions", {}).get("cpis", [])
if len(cpis) != 1 or cpis[0].get("dataLayout") != "system.create_account":
    raise SystemExit(f"CPI schema mismatch: {cpis}")
if cpis[0].get("ownerSource") != "program":
    raise SystemExit(f"CPI owner source mismatch: {cpis}")
print("artifact validation: ok")
PY

echo "=== Solana System create_account CPI step 2: generate local keypairs ==="
...
echo "=== Solana System create_account CPI step 3: start Surfpool ==="
"$SURFPOOL_BIN" start \
  --host "$RPC_HOST" \
  --port "$RPC_PORT" \
  --ws-port "$WS_PORT" \
...
echo "=== Solana System create_account CPI step 4: deploy program ==="
...
echo "=== Solana System create_account CPI step 5: run Rust behavior checks ==="

PROOF_FORGE_SOLANA_RPC_URL="$RPC_URL" \
PROOF_FORGE_SOLANA_PAYER="$PAYER_KEYPAIR" \
PROOF_FORGE_SOLANA_PROGRAM_ID="$PROGRAM_ID" \
  cargo run --manifest-path testkit/Cargo.toml -p proof-forge-testkit-harness-solana --bin system_create_account_cpi_live_smoke \
  || fail "Rust System create_account CPI checks failed"

echo "=== Solana System create_account CPI Surfpool/Rust smoke: PASS ==="
