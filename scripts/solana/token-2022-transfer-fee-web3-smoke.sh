...
# TokenSpec-backed Token-2022 plan, starts Surfpool, then executes transfer-fee
# mint initialization and a checked transfer with fee using @solana/spl-token.
#
# Exit codes:
#   0 - all gates passed
#   1 - a gate failed
...
JS_TEMPLATE="$REPO_ROOT/Tests/solana/token_2022_transfer_fee_plan_live_web3_smoke.mjs"
...
echo "=== Solana Token-2022 transfer-fee live step 4: run Web3.js behavior checks ==="
cp "$JS_TEMPLATE" "$NODE_PROJECT/token_2022_transfer_fee_plan_live_web3_smoke.mjs"
...
PROOF_FORGE_SOLANA_TOKEN_PLAN="$TOKEN_PLAN" \
  node "$NODE_PROJECT/token_2022_transfer_fee_plan_live_web3_smoke.mjs" \
  || fail "Web3.js Solana Token-2022 transfer-fee checks failed"

echo "=== Solana Token-2022 transfer-fee Surfpool/Web3.js smoke: PASS ==="
