//! Scenario: external-token-transfer multi-account (NEP-141 peer client).
...
use crate::host::{
    call_json, call_json_deposit, call_raw, deploy_with_metrics, ensure_file, ensure_ok,
    refresh_storage, view_json_u64,
};
use crate::report::{SideKind, SideReport};

pub(crate) async fn run_external_token_transfer_matrix(
    ...
    println!("external-token-transfer: FT peer = {peer_id}");

    let pf_out =
        repo_root.join("build/testkit/compare/near/external-token-transfer/proof-forge-live");
    if pf_out.exists() {
        let _ = fs::remove_dir_all(&pf_out);
    }
    fs::create_dir_all(&pf_out)?;
    let peer_spec = format!("usdc.peer={peer_id}");
    let status = std::process::Command::new("lake")
        ...
        .arg(&pf_out)
        .arg("Examples/Product/ExternalTokenTransfer.lean")
        .status()
        .context("rebuild ExternalTokenTransfer with peer")?;
    if !status.success() {
        bail!("skip: failed to rebuild PF ExternalTokenTransfer with --peer");
    }
    let pf_wasm_path = ["externaltokentransfer.wasm", "ExternalTokenTransfer.wasm"]
        .iter()
        .map(|n| pf_out.join(n))
        .find(|p| p.is_file())
        .context("peer-rebuilt ExternalTokenTransfer wasm missing")?;
    ...
    let mut pay = 7u64.to_le_bytes().to_vec();
    pay.extend_from_slice(&50u64.to_le_bytes());
    let s = call_raw(&pf_contract, "pay", &pay).await?;
    pf_call_gas = pf_call_gas.saturating_add(s.gas_burnt.unwrap_or(0));
    ensure_ok(&s, "PF pay")?;
    pf_steps.push(s);
    ...
    // Outer call may be payable; attach 0 (promise deposit is separate).
    let s = call_json_deposit(
        &sdk_contract,
        "pay",
        json!({ "recipient": 7, "amount": 50 }),
        0,
    )
    .await?;
    sdk_call_gas = sdk_call_gas.saturating_add(s.gas_burnt.unwrap_or(0));
