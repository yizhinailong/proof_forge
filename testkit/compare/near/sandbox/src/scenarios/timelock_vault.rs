//! Scenario: binary timelock (lock → claim after unlock).
...
pub(crate) async fn run_timelock_vault_side(
    ...
    // unlockAt=1 → any real sandbox block_timestamp fully unlocks amount=1000.
    if ctx.is_pf() {
        ...
        ctx.view_raw_u64("get_unlock_at", "PF unlock", Some(1)).await?;
        ...
        ctx.call_json(
            "lock",
            json!({ "amount": 1000, "unlock_at": 1 }),
            "sdk lock",
        )
        .await?;
        ctx.view_json_u64("get_locked", json!({}), "sdk locked", Some(1000))
            .await?;
        ctx.view_json_u64("get_unlock_at", json!({}), "sdk unlock", Some(1))
            .await?;
